...
class TrainMainCategory(str, Enum):
    """
...
class EffortCurve(BaseModel, extra="forbid"):
    ...
    def check_size(self):
        assert len(self.speeds) == len(self.max_efforts), (
            "speeds and max_efforts must be the same length"
        )
        return self
...
class EffortCurves(BaseModel, extra="forbid"):
    ...
    modes: Mapping[str, ModeEffortCurves] = Field(
    ...
    def check_default_mode(self):
        assert self.default_mode in self.modes, (
...
class EtcsBrakeParams(BaseModel, extra="forbid"):
    ...
    t_bs1: float = Field(
        ge=0, description="T_bs1: time service break in s used for SBI1 computation"
    )
    t_bs2: float = Field(
...
class RefillLaw(BaseModel, extra="forbid"):
    ...
    tau: float = Field(
        ge=0, description="Time constant of the refill behavior (in seconds)"
    )
    soc_ref: float = Field(
        ge=0, le=1, description="Reference (target) value of state of charge"
    )
...
class EnergyStorage(BaseModel, extra="forbid"):
    ...
    capacity: float = Field(
        ge=0, description="How much energy the source can store (in Joules)"
    )
    soc: float = Field(
        ge=0,
        le=1,
    ...
    soc_min: float = Field(
        ge=0, le=1, description="The minimum SoC, where the available energy is zero"
    )
    soc_max: float = Field(
        ge=0,
        le=1,
...
class SpeedDependantPower(BaseModel, extra="forbid"):
    ...
    @model_validator(mode="after")
    def check_size(self):
        assert len(self.speeds) == len(self.powers), (
            "speeds and powers must have the same length"
        )
        return self
...
class EnergySource(RootModel):
    ...
    root: Union[Electrification, PowerPack, Battery] = Field(
        discriminator="energy_source_type"
    )
...
class RollingStock(BaseModel, extra="forbid"):
    ...
    * Then, speed-effort curves can be computed for all electrical profile values.
    ...
    rolling_resistance: RollingResistance = Field(
    ...
    other_categories: List[TrainMainCategory] = Field(
...
class TowedRollingStock(BaseModel, extra="forbid"):
    ...
    max_speed: Optional[NonNegativeFloat] = Field(
        description="The maximum speed of the train, in m/s"
    )
    comfort_acceleration: NonNegativeFloat = Field(
        description="The maximum operational acceleration in m/s^2"
    )
