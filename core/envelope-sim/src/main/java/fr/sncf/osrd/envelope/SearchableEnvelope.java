...
/**
 * A utility class to search envelope positions, both on envelopes and envelope parts. It performs
 * the search on the position of points if on envelope parts, and on the position of envelope parts
 * if on envelopes.
 *
 * <p>The position arrays encodes a 1D monotonic polyline:
 *
 * <pre>
 *     +------------+--------+----------+
 *     0            1        2          3
...
public interface SearchableEnvelope {
    ...
    default int findLeft(double position) {
        ...
        if (insertionPoint == 0 || insertionPoint == positionPointsCount()) return -insertionPoint - 1;
    ...
    default int findRight(double position) {
        ...
        // if the position matches one of the data points, return the range on the left, if any
        if (pointIndex >= 0) {
            if (pointIndex == positionPointsCount() - 1) return pointIndex - 1;
            return pointIndex;
        }
    ...
    default int findLeftDir(double position, double direction) {
        if (direction < 0) return findRight(position);
    ...
    default int findRightDir(double position, double direction) {
        if (direction < 0) return findLeft(position);
