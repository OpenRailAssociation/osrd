...
public class EnvelopePhysics {
    ...
    public static double stepAcceleration(double lastPos, double nextPos, double lastSpeed, double nextSpeed) {
    ...
    public static double interpolateStepTime(double lastPos, double nextPos, double lastSpeed, double nextSpeed) {
        var positionDelta = nextPos - lastPos;
        var acceleration = stepAcceleration(lastPos, nextPos, lastSpeed, nextSpeed);
        if (acceleration == 0.0) return Math.abs(positionDelta / lastSpeed);
     ...
     * over <b>time</b> inside a step. Interpolation thus needs to be done on a parabola, as
     * envelopes are over <b>space</b>. The intersection is guaranteed to be within the bounds of
     * both segments.
     */
    ...
    public static double intersectStepWithSpeed(
            double a1Pos, double a1Speed, double a2Pos, double a2Speed, double bSpeed) {
        double accA = stepAcceleration(a1Pos, a2Pos, a1Speed, a2Speed);
    ...
    public static double getMechanicalEnergyConsumed(
            Envelope envelope, PhysicsPath path, PhysicsRollingStock rollingStock) {
    ...
    public static double getPartMechanicalEnergyConsumed(
            EnvelopePart part, PhysicsPath path, PhysicsRollingStock rollingStock) {
