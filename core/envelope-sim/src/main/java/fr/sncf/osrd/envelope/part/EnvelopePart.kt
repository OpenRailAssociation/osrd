...
class EnvelopePart(
    ...
    val maxSpeed: Double
        ...
        get() {
            if (!java.lang.Double.isNaN(maxSpeedCache)) return maxSpeedCache

            var maxSpeed = Double.NEGATIVE_INFINITY
            for (speed in speeds) if (speed > maxSpeed) maxSpeed = speed
    ...
    val minSpeed: Double
        ...
        get() {
            if (!java.lang.Double.isNaN(minSpeedCache)) return minSpeedCache

            var minSpeed = Double.POSITIVE_INFINITY
            for (speed in speeds) if (speed < minSpeed) minSpeed = speed
    ...
    val totalTimeUS: Long
        ...
        get() {
            val totalTimes = totalTimesUS
            return totalTimes[totalTimes.size - 1]
        }
    ...
    fun slice(
        ...
        if (
            newBeginStepIndex == 0 &&
                ...
                newBeginPos == Double.NEGATIVE_INFINITY &&
                newEndPos == Double.POSITIVE_INFINITY &&
                java.lang.Double.isNaN(newBeginSpeed) &&
                java.lang.Double.isNaN(newEndSpeed)
        )
            return this
        ...
        if (newEndPos != Double.POSITIVE_INFINITY) {
            if (java.lang.Double.isNaN(newEndSpeed))
                newEndSpeed = interpolateSpeed(newEndStepIndex, newEndPos)
        ...
        if (newBeginPos != Double.NEGATIVE_INFINITY) {
            if (java.lang.Double.isNaN(newBeginSpeed))
        ...
        if (!java.lang.Double.isNaN(newBeginSpeed)) sliced.speeds[0] = newBeginSpeed
        if (!java.lang.Double.isNaN(newEndSpeed))
            sliced.speeds[sliced.pointCount() - 1] = newEndSpeed
    ...
    fun copyAndShift(
        ...
        for (i in 1 until positions.size) {
            ...
            if (newPositions[newPositions.size() - 1] != p) {
    ...
    companion object {
        ...
        private fun checkNaNFree(values: DoubleArray): Boolean {
            for (`val` in values) if (java.lang.Double.isNaN(`val`)) return false
            return true
        }
