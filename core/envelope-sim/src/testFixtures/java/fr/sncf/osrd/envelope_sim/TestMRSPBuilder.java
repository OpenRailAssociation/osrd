...
public class TestMRSPBuilder {
    ...
    public static Envelope makeSimpleMRSP(EnvelopeSimContext context, double speed) {
    ...
    public static Envelope makeSimpleMRSP(EnvelopeSimContext context, RangeMap<Double, Double> speeds) {
    ...
    public static Envelope makeComplexMRSP(EnvelopeSimContext context) {
        assert context.path.getLength() >= 100000 : "Path length must be greater than 100km to generate a complex MRSP";
        var builder = new MRSPEnvelopeBuilder();
        var maxSpeedRS = context.rollingStock.getMaxSpeed();
        builder.addPart(EnvelopePart.generateTimes(
                List.of(EnvelopeProfile.CONSTANT_SPEED),
        ...
        BiConsumer<double[], double[]> addSpeedLimit = (double[] positions, double[] speeds) ->
                builder.addPart(EnvelopePart.generateTimes(List.of(EnvelopeProfile.CONSTANT_SPEED), positions, speeds));
