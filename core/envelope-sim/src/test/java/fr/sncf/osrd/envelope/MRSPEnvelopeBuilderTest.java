...
class MRSPEnvelopeBuilderTest {
    ...
    public void speedLimitByCategory() {
        ...
        EnvelopeTestUtils.assertEquals(
                Envelope.make(
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 0, 5, 50),
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 5, 30, 25),
                        makeFlatPart(List.of(
                                LimitKind.TRAIN_LIMIT, CONSTANT_SPEED), 30, 80, TestTrains.REALISTIC_FAST_TRAIN.maxSpeed)
                ), mrspNoTags, 0.001);

        EnvelopeTestUtils.assertEquals(
                Envelope.make(
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 0, 5, 75),
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 5, 30, 25),
                        makeFlatPart(List.of(
                                LimitKind.TRAIN_LIMIT, CONSTANT_SPEED), 30, 80, TestTrains.REALISTIC_FAST_TRAIN.maxSpeed)
                ), mrspFast, 0.001);
        ...
        EnvelopeTestUtils.assertEquals(
                Envelope.make(
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 0, 5, 50),
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 5, 30, 10),
                        makeFlatPart(List.of(
                                LimitKind.TRAIN_LIMIT, CONSTANT_SPEED), 30, 80, TestTrains.REALISTIC_FAST_TRAIN.maxSpeed)
                ), mrspShort, 0.001);
        ...
        EnvelopeTestUtils.assertEquals(
                Envelope.make(
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 0, 5, 22),
                        makeFlatPart(List.of(LimitKind.SPEED_LIMIT, CONSTANT_SPEED), 5, 30, 25),
                        makeFlatPart(List.of(
                                LimitKind.TRAIN_LIMIT, CONSTANT_SPEED), 30, 80, TestTrains.REALISTIC_FAST_TRAIN.maxSpeed)
                ), mrspLong, 0.001);
