 ...
 *
 * This implementation is *not* optimized, deep copies are made at every call.
 * It should eventually be improved by implementing actual views on DistanceRangeMaps.
 * This should be done as part of the refactor that integrates STDCM with conflict detection,
...
data class PathPropertiesView(
    ...
    override fun getOperationalPointParts(): List<IdxWithPathOffset<OperationalPointPart>> {
        return base.getOperationalPointParts()
    ...
    override fun getGeo(): LineString {
        ...
        return base.getGeo().slice(
            startOffset.distance.meters / baseLength,
            endOffset.distance.meters / baseLength
        )
    }
    ...
    override fun getCatenary(): DistanceRangeMap<String> {
        return sliceRangeMap(base.getCatenary())
    }
    ...
    override fun getTrackLocationOffset(location: TrackLocation): Offset<Path>? {
        ...
        if (baseResult < startOffset || baseResult > endOffset)
            return null
        return baseResult - startOffset.distance
    }

    override fun <T> getRangeMapFromUndirected(getData: (chunkId: TrackChunkId) -> DistanceRangeMap<T>): DistanceRangeMap<T> {
    ...
    private fun <T> sliceRangeMap(map: DistanceRangeMap<T>): DistanceRangeMap<T> {
        if (map.isEmpty() || (startOffset.distance == 0.meters && endOffset.distance == map.upperBound()))
            return map
        val res = map.clone()
