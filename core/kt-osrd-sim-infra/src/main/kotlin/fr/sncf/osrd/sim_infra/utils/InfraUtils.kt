...
fun BlockInfra.chunksOnBlocks(blockIds: List<BlockId>): DirStaticIdxList<TrackChunk> {
    ...
    for (block in blockIds)
        for (chunk in getTrackChunksFromBlock(block))
            res.add(chunk)
    return res
}
...
fun BlockInfra.routesOnBlock(rawInfra: RawInfra, block: BlockId): StaticIdxList<Route> {
    ...
    for (routeId in routes)
        if (routeMatchPath(rawInfra, this, chunks, 0, false, routeId))
            res.add(routeId)
    return res
}
...
private fun routeHasBlockPath(
    ...
    val blockPaths = recoverBlocks(
        infra, blockInfra, mutableStaticIdxArrayListOf(routeId), null
    )
