...
public class BacktrackingTests {
    ...
    public void testBacktrackingBrakingCurve() {
        ...
        var block = infra.addBlock("a", "b", 1000_000);
        ...
        var occupancyGraph = ImmutableMultimap.of(
                block, new OccupancySegment(runTime + 1, POSITIVE_INFINITY, 0, 1000_000)
        );
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(block, 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(block, 1_000_000)))
    ...
    public void testBacktrackingBrakingCurveSeveralBlocks() {
        ...
        infra.addBlock("b", "c", 10_000);
        infra.addBlock("c", "d", 10_000);
        var lastBlock = infra.addBlock("d", "e", 10_000);
        var firstBlockEnvelope = STDCMSimulations.simulateBlock(infra, infra,
                firstBlock, 0, 0, REALISTIC_FAST_TRAIN, RollingStock.Comfort.STANDARD, 2., null, null);
        assert firstBlockEnvelope != null;
        var runTime = firstBlockEnvelope.getTotalTime();
        var occupancyGraph = ImmutableMultimap.of(
                lastBlock, new OccupancySegment(runTime + 10, POSITIVE_INFINITY, 0, 10_000)
        );
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(firstBlock, 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(lastBlock, 5_000)))
                .setUnavailableTimes(occupancyGraph)
                .run();
        if (res == null)
            return;
    ...
    public void testBacktrackingMRSPDrop() {
        ...
        var infra = DummyInfra.make();
        var firstBlock = infra.addBlock("a", "b", 1000_000);
        var secondBlock = infra.addBlock("b", "c", 100_000, 5);
        var firstBlockEnvelope = STDCMSimulations.simulateBlock(infra, infra,
                firstBlock, 0, 0, REALISTIC_FAST_TRAIN, RollingStock.Comfort.STANDARD, 2., null, null);
        assert firstBlockEnvelope != null;
        var runTime = firstBlockEnvelope.getTotalTime();
        var occupancyGraph = ImmutableMultimap.of(
                firstBlock, new OccupancySegment(runTime + 10, POSITIVE_INFINITY, 0, 1000_000)
        );
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(firstBlock, 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(secondBlock, 5_000)))
                .setUnavailableTimes(occupancyGraph)
                .run();
        if (res == null)
            return;
    ...
    public void testManyBacktracking() {
        ...
        infra.addBlock("b", "c", 10_000, 20);
        infra.addBlock("c", "d", 10_000, 15);
        infra.addBlock("d", "e", 10_000, 10);
        var lastBlock = infra.addBlock("e", "f", 1000_000, 5);
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(firstBlock, 0)))
