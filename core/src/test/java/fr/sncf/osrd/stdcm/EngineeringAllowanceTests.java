...
public class EngineeringAllowanceTests {
    ...
    public void testSlowdown() {
        ...
        var firstBlock = infra.addBlock("a", "b", 1_000_000, 30);
        var secondBlock = infra.addBlock("b", "c", 10_000_000, 30);
        var thirdBlock = infra.addBlock("c", "d", 100_000, 30);
        ...
        var occupancyGraph = ImmutableMultimap.of(
                firstBlock, new OccupancySegment(firstBlockEnvelope.getTotalTime() + 10, POSITIVE_INFINITY,
                        0, 1_000_000),
                thirdBlock, new OccupancySegment(0, timeThirdBlockFree + 30, 0, 100_000)
        );
        double timeStep = 2;
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(firstBlock, 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(thirdBlock, 1_000)))
    ...
    public void testSlowdownSeveralBlocks() {
        ...
        final var firstBlock = infra.addBlock("a", "b", 1_000_000, 20);
        final var secondBlock = infra.addBlock("b", "c", 1_000_000, 20);
        infra.addBlock("c", "d", 1_000_000, 20);
        infra.addBlock("d", "e", 1_000_000, 20);
        var lastBlock = infra.addBlock("e", "f", 1_000_000, 20);
        var firstBlockEnvelope = STDCMSimulations.simulateBlock(infra, infra,
                firstBlock, 0, 0,
                REALISTIC_FAST_TRAIN, RollingStock.Comfort.STANDARD, 2., null, null);
        assert firstBlockEnvelope != null;
        ...
        var occupancyGraph = ImmutableMultimap.of(
                firstBlock, new OccupancySegment(firstBlockEnvelope.getTotalTime() + timeStep,
                        POSITIVE_INFINITY, 0, 1_000_000),
                lastBlock, new OccupancySegment(0, timeLastBlockFree, 0, 1_000_000)
        );
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(firstBlock, 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(lastBlock, 1_000_000)))
                .setUnavailableTimes(occupancyGraph)
                .setTimeStep(timeStep)
                .run();
    ...
    public void testSlowdownWithConflicts() {
        ...
        final var firstBlock = infra.addBlock("a", "b", 1_000_000, 20);
        final var secondBlock = infra.addBlock("b", "c", 1_000_000, 20);
        final var thirdBlock = infra.addBlock("c", "d", 1_000_000, 20);
        infra.addBlock("d", "e", 1_000_000, 20);
        var lastBlock = infra.addBlock("e", "f", 1_000_000, 20);
        var firstBlockEnvelope = STDCMSimulations.simulateBlock(infra, infra,
                firstBlock, 0, 0,
        ...
        var occupancyGraph = ImmutableMultimap.of(
                firstBlock, new OccupancySegment(firstBlockEnvelope.getTotalTime() + timeStep,
                        POSITIVE_INFINITY, 0, 1_000_000),
                lastBlock, new OccupancySegment(0, timeLastBlockFree, 0, 1_000_000),
                thirdBlock, new OccupancySegment(timeThirdBlockOccupied, POSITIVE_INFINITY, 0, 1_000_000)
        );
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(firstBlock, 0)))
    ...
    public void testMaxDepartureTimeShift() {
     ...
     max delay at departure time
        ...
        var infra = DummyInfra.make();
        var firstBlock = infra.addBlock("a", "b", 1_000_000, 30);
        var secondBlock = infra.addBlock("b", "c", 1_000_000, 30);
        var thirdBlock = infra.addBlock("c", "d", 1_000, 30);
        ...
        var occupancyGraph = ImmutableMultimap.of(
                thirdBlock, new OccupancySegment(0, timeThirdBlockFree, 0, 1_000)
        );
        double timeStep = 2;
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
    ...
    public void testAllowanceWithDepartureTimeShift() {
         ...
         */
        var infra = DummyInfra.make();
        var firstBlock = infra.addBlock("a", "b", 2_000_000, 20);
        var secondBlock = infra.addBlock("b", "c", 2_000_000, 20);
        var thirdBlock = infra.addBlock("c", "d", 2_000_000, 20);
        var forthBlock = infra.addBlock("d", "e", 2_000_000, 20);
        var occupancyGraph = ImmutableMultimap.of(
                firstBlock, new OccupancySegment(0, 600, 0, 100_000),
                firstBlock, new OccupancySegment(2_000, POSITIVE_INFINITY, 0, 100_000),
                secondBlock, new OccupancySegment(0, 1200, 0, 100_000),
                thirdBlock, new OccupancySegment(0, 1800, 0, 100_000),
                forthBlock, new OccupancySegment(0, 4_000, 0, 100_000)
        );
        double timeStep = 2;
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(firstBlock, 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(forthBlock, 1_000)))
                .setUnavailableTimes(occupancyGraph)
                .setTimeStep(timeStep)
                .run();
        assertNotNull(res);
    ...
    public void testImpossibleEngineeringAllowance() {
        ...
        var infra = DummyInfra.make();
        var blocks = List.of(
                infra.addBlock("a", "b", 1_000_000),
                infra.addBlock("b", "c", 1_000),
                infra.addBlock("c", "d", 1_000_000)
        );
        var occupancyGraph = ImmutableMultimap.of(
                blocks.get(0), new OccupancySegment(300, POSITIVE_INFINITY, 0, 1_000_000),
                blocks.get(2), new OccupancySegment(0, 3600, 0, 1_000_000)
        );
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(blocks.get(0), 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(blocks.get(2), 1_000_000)))
                .setUnavailableTimes(occupancyGraph)
    ...
    public void testReturnTheFastestPathWithAllowance() {
        ...
        var occupancyGraph = ImmutableMultimap.of(
                blocks.get(0), new OccupancySegment(300, 3600, 0, 1_000),
        ...
        var res = new STDCMPathfindingBuilder()
                .setInfra(infra.fullInfra())
                .setStartLocations(Set.of(new Pathfinding.EdgeLocation<>(blocks.get(0), 0)))
                .setEndLocations(Set.of(new Pathfinding.EdgeLocation<>(blocks.get(2), 100_000)))
                .setUnavailableTimes(occupancyGraph)
                .setTimeStep(timeStep)
                .run();
