...
class SpacingResourceGeneratorTest {
    ...
    fun setupTests() {
        ...
        blockRanges =
            buildRangeList(
                blocks.map {
                    PartialBlockRange(it, Offset.zero(), infra.blockInfra.getBlockLength(it))
                }
            )
        routeRanges =
            buildRangeList(
                routes.map {
                    PartialRouteRange(it, Offset.zero(), infra.rawInfra.getRouteLength(it))
                }
            )
        zoneRanges =
            blockRanges
    ...
    fun testDifferentPathLengths() {
        ...
        val automaton =
            SpacingResourceGenerator(
                ...
                infra.signalingSimulator,
                null,
                makeCallbacks(blockRanges[0].pathEnd, false),
            )
