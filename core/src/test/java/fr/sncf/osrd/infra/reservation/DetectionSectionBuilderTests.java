...
public class DetectionSectionBuilderTests {
    ...
    public void testDetectionSwitchInfra() {
        ...
        setDetectors(infra, "1", List.of(new OffsetIDPair(10, "D1-inner"), new OffsetIDPair(15, "D1-outer")));
        setDetectors(infra, "2", List.of(new OffsetIDPair(20, "D2")));
        setDetectors(infra, "3", List.of(new OffsetIDPair(30, "D3-inner"), new OffsetIDPair(40, "D3-outer")));
    ...
    /**
     * Converts a list of section into a set of (set of (direction, detector id)) for easier
     * equality testing. Each inner set corresponds to a detection section
     */
    private Set<Set<DirIDPair>> convertSections(ArrayList<DetectionSection> sections) {
        var res = new HashSet<Set<DirIDPair>>();
        for (var s : sections)
            res.add(s.getDetectors().stream()
    ...
    private void setDetectors(TrackInfra infra, String tracKID, List<OffsetIDPair> objects) {
        ...
        for (var detector : objects) list.add(new DetectorImpl(track, detector.offset, false, detector.id));
