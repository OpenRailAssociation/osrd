...
public class RJSParsingTests {
    ...
    public void testTinyInfra() throws Exception {
        ...
        var fooAEndpoint =
                graph.incidentNodes(getTrack(infra, "ne.micro.foo_a")).nodeU();
        var fooBEndpoint =
                graph.incidentNodes(getTrack(infra, "ne.micro.foo_b")).nodeU();
        var barAEndpoint =
                graph.incidentNodes(getTrack(infra, "ne.micro.bar_a")).nodeU();
    ...
    public void testDuplicatedSwitch() throws Exception {
        ...
        var thrown = assertThrows(
                OSRDError.class, () -> UndirectedInfraBuilder.parseInfra(rjsInfra, new DiagnosticRecorderImpl(true)));
    ...
    public void testOverlappingSpeedSections() throws Exception {
        ...
        var speedLimits =
                parsedInfra.getTrackSection(track.id).getSpeedSections().get(Direction.FORWARD);
