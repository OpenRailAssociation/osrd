...
class STDCMEndpointTest : ApiTest() {
    ...
    fun simpleEmptyTimetable() {
        val requestBody = STDCMRequest.adapter.toJson(
            STDCMRequest(
                "tiny_infra/infra.json",
                "1",
                getExampleRollingStock("fast_rolling_stock.json"), setOf(),
                listOf(
        ...
        val result = TakesUtils.readBodyResponse(
            STDCMEndpoint(infraManager).act(
                RqFake("POST", "/stdcm", requestBody)
            )
        )
        val response = STDCMResponse.adapter.fromJson(result)!!
        val routes = response.path.routePaths.stream()
            .map { route: RJSRoutePath? -> route!!.route }
            .toList()
