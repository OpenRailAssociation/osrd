...
class FullSTDCMTests {
    ...
    fun testTinyInfra() {
        ...
        STDCMPathfindingBuilder()
            .setInfra(infra)
            .setRollingStock(
                RJSRollingStockParser.parse(Helpers.getExampleRollingStock("fast_rolling_stock.json"))
            )
            .setStartLocations(
                setOf(
    ...
    private fun checkNoConflict(infra: FullInfra, requirements: List<SpacingRequirement>, res: STDCMResult) {
        val requirementMap = HashMultimap.create<String, SpacingRequirement>()
        for (requirement in requirements) {
            requirementMap.put(requirement.zone, requirement)
        }
        val newRequirements = run(
            res.envelope,
            res.trainPath,
            res.chunkPath,
            makeTrainSchedule(
                res.envelope.endPos, TestTrains.REALISTIC_FAST_TRAIN,
