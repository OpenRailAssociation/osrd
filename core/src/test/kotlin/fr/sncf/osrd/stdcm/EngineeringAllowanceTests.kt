...
class EngineeringAllowanceTests {
    ...
    fun testSlowdown() {
        ...
        val occupancyGraph = ImmutableMultimap.of(
            firstBlock, OccupancySegment(
                firstBlockEnvelope.totalTime + 10, Double.POSITIVE_INFINITY,
                0.meters, 1000.meters
            ),
            thirdBlock, OccupancySegment(0.0, timeThirdBlockFree + 30, 0.meters, 100.meters)
        )
        val timeStep = 2.0
    ...
    fun testSlowdownSeveralBlocks() {
        ...
        val occupancyGraph = ImmutableMultimap.of(
            firstBlock, OccupancySegment(
                ...
                Double.POSITIVE_INFINITY, 0.meters, 1000.meters
            ),
            lastBlock, OccupancySegment(0.0, timeLastBlockFree, 0.meters, 1000.meters)
        )
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
            .setStartLocations(setOf(EdgeLocation(firstBlock, Offset<Block>(0.meters))))
    ...
    fun testSlowdownWithConflicts() {
        ...
        val occupancyGraph = ImmutableMultimap.of(
            ...
            thirdBlock,
            OccupancySegment(timeThirdBlockOccupied, Double.POSITIVE_INFINITY, 0.meters, 1000.meters)
        )
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
            .setStartLocations(setOf(EdgeLocation(firstBlock, Offset<Block>(0.meters))))
    ...
    fun testMaxDepartureTimeShift() {
     ...
     max delay at departure time
        ...
        val lastBlockEntryTime = getBlocksRunTime(
            infra.fullInfra(),
            listOf(firstBlock, secondBlock)
        )
        val timeThirdBlockFree = lastBlockEntryTime + 3600 * 2 + 60
        val occupancyGraph = ImmutableMultimap.of(
            thirdBlock, OccupancySegment(0.0, timeThirdBlockFree, 0.meters, 1.meters)
        )
        val timeStep = 2.0
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
    ...
    fun testAllowanceWithDepartureTimeShift() {
        ...
        val occupancyGraph = ImmutableMultimap.of(
            firstBlock, OccupancySegment(0.0, 600.0, 0.meters, 100.meters),
            firstBlock, OccupancySegment(2000.0, Double.POSITIVE_INFINITY, 0.meters, 100.meters),
            secondBlock, OccupancySegment(0.0, 1200.0, 0.meters, 100.meters),
            thirdBlock, OccupancySegment(0.0, 1800.0, 0.meters, 100.meters),
            forthBlock, OccupancySegment(0.0, 4000.0, 0.meters, 100.meters)
        )
        val timeStep = 2.0
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
    ...
    fun testImpossibleEngineeringAllowance() {
        ...
        val occupancyGraph = ImmutableMultimap.of(
            blocks[0], OccupancySegment(300.0, Double.POSITIVE_INFINITY, 0.meters, 1000.meters),
            blocks[2], OccupancySegment(0.0, 3600.0, 0.meters, 1000.meters)
        )
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
    ...
    fun testReturnTheFastestPathWithAllowance() {
        ...
        val occupancyGraph = ImmutableMultimap.of(
            blocks[0], OccupancySegment(300.0, 3600.0, 0.meters, 1.meters),
            blocks[2], OccupancySegment(0.0, 3600.0, 0.meters, 1.meters)
        )
        val timeStep = 2.0
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
