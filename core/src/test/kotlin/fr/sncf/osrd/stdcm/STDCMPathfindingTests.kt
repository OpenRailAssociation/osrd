...
class STDCMPathfindingTests {
    ...
    fun betweenTrains() {
        ...
        val occupancyGraph = ImmutableMultimap.of(
            firstBlock, OccupancySegment(0.0, 50.0, 0.meters, 100.meters),
            firstBlock, OccupancySegment(10000.0, Double.POSITIVE_INFINITY, 0.meters, 100.meters),
            secondBlock, OccupancySegment(0.0, 50.0, 0.meters, 100.meters),
            secondBlock, OccupancySegment(10000.0, Double.POSITIVE_INFINITY, 0.meters, 100.meters)
        )
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
            .setStartTime(100.0)
    ...
    fun impossiblePath() {
        ...
        val secondBlock = infra.addBlock("b", "c")
        val occupancyGraph = ImmutableMultimap.of(
            firstBlock, OccupancySegment(0.0, 99.0, 0.meters, 100.meters),
            firstBlock, OccupancySegment(101.0, Double.POSITIVE_INFINITY, 0.meters, 100.meters),
            secondBlock, OccupancySegment(0.0, 50.0, 0.meters, 100.meters),
            secondBlock, OccupancySegment(1000.0, Double.POSITIVE_INFINITY, 0.meters, 100.meters)
        )
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
            .setStartTime(100.0)
    ...
    fun lastBlockOccupiedAtStart() {
        ...
        val secondBlock = infra.addBlock("b", "c")
        val occupancyGraph = ImmutableMultimap.of(
            secondBlock, OccupancySegment(0.0, 10.0, 0.meters, 100.meters)
        )
        val res = STDCMPathfindingBuilder()
            .setInfra(infra.fullInfra())
            .setStartLocations(setOf(EdgeLocation(firstBlock, 0.meters)))
    ...
    fun testAvoidBlockedBlocks() {
        ...
        val occupancyGraph1 = ImmutableMultimap.of(
            blockTop, OccupancySegment(0.0, Double.POSITIVE_INFINITY, 0.meters, 100.meters)
        )
        val occupancyGraph2 = ImmutableMultimap.of(
            blockBottom, OccupancySegment(0.0, Double.POSITIVE_INFINITY, 0.meters, 100.meters)
        )
        ...
        val blocks1 = res1.blocks.ranges.stream()
            ...
            .toList()
        val blocks2 = res2.blocks.ranges.stream()
            .map { block -> infra.blockPool[block.edge.index.toInt()].name }
            .toList()
