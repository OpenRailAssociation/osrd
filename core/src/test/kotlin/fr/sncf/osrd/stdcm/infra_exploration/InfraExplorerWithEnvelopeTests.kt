...
import fr.sncf.osrd.train.TestRollingStock
...
class InfraExplorerWithEnvelopeTests {
    ...
    fun testTwoDifferentPaths() {
        ...
        extended[0].addEnvelope(Envelope.make(
        ...
        extended[1].addEnvelope(Envelope.make(
            EnvelopePart.generateTimes(
                listOf(EnvelopeProfile.CONSTANT_SPEED),
                doubleArrayOf(0.0, 100.0),
        ...
        testEnvelopeTimeEquality(
            Envelope.make(EnvelopePart.generateTimes(
                listOf(EnvelopeProfile.CONSTANT_SPEED),
    ...
    fun testClone() {
        ...
        explorer.addEnvelope(Envelope.make(
            EnvelopePart.generateTimes(
                listOf(EnvelopeProfile.CONSTANT_SPEED),
                doubleArrayOf(0.0, 100.0),
                doubleArrayOf(30.0, 30.0)
        ...
        cloned.addEnvelope(Envelope.make(
            EnvelopePart.generateTimes(
                listOf(EnvelopeProfile.CONSTANT_SPEED),
                doubleArrayOf(0.0, 100.0),
    ...
    /** Test that the envelope bounds are equal,
     * then iterate by an arbitrary step length to test for interpolateTotalTime equality. */
