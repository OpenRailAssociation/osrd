...
class PathfindingResultConverterTest {
    ...
    fun testMakePathForward() {
        ...
        val blocks = Helpers.getBlocksOnRoutes(
            infra, listOf(
                "rt.DA2->DA5",
                "rt.DA5->DC5"
            )
    ...
    fun testPathWaypointOnLoop() {
        ...
        val userDefinedWaypoints = waypoints.stream()
    ...
    companion object {
        private fun checkBlocks(
            ...
            for (dirChunk in path.chunks) {
                val trackName =
                    infra.rawInfra.getTrackSectionName(infra.rawInfra.getTrackFromChunk(dirChunk.value))
