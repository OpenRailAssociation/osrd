...
public class UnavailableSpaceBuilder {
    ...
    public static Multimap<Integer, OccupancySegment> computeUnavailableSpace(
        ...
        for (var occupancy : occupancies) {
            ...
            var length = rawInfra.getRouteLength(route);
            ...
            addRouteOccupancy(unavailableSpace, rawInfra, blockInfra, route, timeStart, timeEnd, 0, length);

            //Generating sight Distance occupancy
            var predecessorRoutes = getPreviousRoutes(rawInfra, route);
            for (var predecessorRoute : predecessorRoutes) {
                var preBlockLength = rawInfra.getRouteLength(predecessorRoute);
                addRouteOccupancy(
                        unavailableSpace,
                        rawInfra,
                        blockInfra,
            ...
            var successorRoutes = getNextRoutes(rawInfra, route);
            for (var successorRoute : successorRoutes) {
                var successorBlockLength = rawInfra.getRouteLength(successorRoute);
                addRouteOccupancy(
                        unavailableSpace,
                        rawInfra,
                        blockInfra,
            ...
            for (var successorRoute : successorRoutes) {
                var secondSuccessorRoutes = getNextRoutes(rawInfra, successorRoute);
                for (var secondSuccessorRoute : secondSuccessorRoutes) {
                    var end = Math.min(
                            Distance.fromMeters(rollingStock.getLength()),
                            rawInfra.getRouteLength(secondSuccessorRoute)
                    );
                    addRouteOccupancy(unavailableSpace, rawInfra, blockInfra, secondSuccessorRoute,
