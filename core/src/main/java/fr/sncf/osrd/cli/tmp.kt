...
fun foo(rawInfra: RawInfra, blockInfra: BlockInfra) {
    ...
    for ((a, b, used) in links) {
        ...
        val number1 =
            rawInfra.getTrackChunkTrackNumber(rawInfra.getTrackSectionChunks(a.value).first())
        val number2 =
            rawInfra.getTrackChunkTrackNumber(rawInfra.getTrackSectionChunks(b.value).first())
