    ...
    @Parameter(
        ...
        description =
            "Path to a JSON file containing the requirements, generated by " +
                "`download-stdcm-requirements.py`. Using the cache (json.gz) file is recommended if available.",
    )
    private var jsonTimetable: String? = null
    @Parameter(
        names = ["--cache-timetable"],
        description =
            "Path to a cache file containing the requirements, can be cached by `TimetableCacheManager` or fetched from the s3",
    )
