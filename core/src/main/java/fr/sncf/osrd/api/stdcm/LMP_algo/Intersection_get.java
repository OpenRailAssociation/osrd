...
public class Intersection_get {
    public ArrayList<ArrayList<ArrayList<BlockUse>>> intersection (ArrayList<ArrayList<ArrayList<BlockUse>>> Get1) throws ParseException {
        ...
        for(index=0;index<B2free.size();index++) {
             for(int i=0;i<B22free.size();i++) {

                var main_block=B2free.get(index);
                var intersected_block=B22free.get(i);
            ...
            if(test==true&&index==B2free.size()-1) {
                index=-1;
                test=false;
              }
        }
        ...
        for (int i=0;i<Get1.size();i++) {
            Get1free.add(new ArrayList<ArrayList<BlockUse>>());
            for(int j=0;j<Get1.get(i).size();j++)
                Get1free.get(i).add(new ArrayList<BlockUse>());
        }
      return Get1free;
    }
}
