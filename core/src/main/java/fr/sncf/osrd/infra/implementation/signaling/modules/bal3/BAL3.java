...
/**
 * This module implements the BAL3 signaling system. It will eventually be moved to an external
 * module
 */
public class BAL3 implements SignalingModule {
    ...
    public record BAL3Route(ReservationRoute infraRoute, Signal<?> entrySignal, Signal<?> exitSignal)
    ...
    private DiDetector findAssociatedDetector(ReservationInfra infra, RJSSignal signal) {
        ...
        if (signal.direction == EdgeDirection.START_TO_STOP) {
            ...
            for (var detector : trackSection.getDetectors()) {
                if (detector.getOffset() < signalPosition) continue;
                if (detector.getOffset() >= signalPosition) return detector.getDiDetector(Direction.FORWARD);
            }
            ...
            for (var detector : trackSection.getDetectors().reverse()) {
                if (detector.getOffset() > signalPosition) continue;
                if (detector.getOffset() <= signalPosition) return detector.getDiDetector(Direction.BACKWARD);
            }
            // if that fails, look for linked track sections
    ...
    public ImmutableMap<RJSSignal, Signal<?>> createSignals(ReservationInfra infra, RJSInfra rjsInfra) {
        ...
        for (var signal : rjsInfra.signals) {
            if (!isBALSignal(signal)) continue;
            // automatically find the associated detector (all BAL3 signals are supposed to have
            // one)
            var linkedDetector = findAssociatedDetector(infra, signal);
            if (linkedDetector == null)
                diagnosticRecorder.register(
                        new Warning(String.format("Can't find associated detector for signal %s", signal.id)));
    ...
    public ImmutableMap<ReservationRoute, SignalingRoute> createRoutes(
            ReservationInfra infra, ImmutableMultimap<RJSSignal, Signal<?>> signalMap) {
        ...
        for (var infraRoute : graph.edges()) {
            if (!isBALRoute(infraRoute)) continue;
            ...
            if (startSignal != null) {
                ...
                if (endSignal != null) startSignal.signalDependencies.add(endSignal);
            }
        }
        for (var signal : signalMap.values()) if (signal instanceof BAL3Signal bal3Signal) bal3Signal.setup();
        return res.build();
    }
