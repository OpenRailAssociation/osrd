...
public final class TrackRangeView {
    ...
    /**
     * A pair containing an element and an offset. The offset is oriented with 0 = start of the
     * range. The offset contained in the element is based on the track itself, it may be different
     */
    public record ElementView<T>(double offset, T element) {}

    private static final Comparator<? super ElementView<?>> comparator = Comparator.comparingDouble(x -> x.offset);
    ...
    /**
     * Returns the speed sections with positions referring to the track range (0 = directed start of
     * the range)
     */
    ...
    /**
     * Returns the gradients with positions referring to the track range (0 = directed start of the
     * range)
     */
    ...
    /**
     * Returns the curves with positions referring to the track range (0 = directed start of the
     * range)
     */
    ...
    /**
     * Returns the slopes with positions referring to the track range (0 = directed start of the
     * range)
     */
    ...
    public LineString getGeo() {
        ...
        if (track.getDirection().equals(Direction.FORWARD)) return forwardResult;
        else return forwardResult.reverse();
    }
    ...
    public TrackLocation offsetLocation(double offset) {
        ...
        if (track.getEdge() instanceof TrackSection ts) trackSection = ts;
        if (track.getDirection().equals(Direction.FORWARD)) return new TrackLocation(trackSection, begin + offset);
        else return new TrackLocation(trackSection, end - offset);
    }
    ...
    public TrackRangeView truncateBegin(double offset) {
        ...
        if (track.getDirection().equals(Direction.FORWARD)) return new TrackRangeView(offset, end, track);
        else return new TrackRangeView(begin, offset, track);
    }

    /** Returns a new view where the end is truncated from the given offset */
    public TrackRangeView truncateEnd(double offset) {
        offset = Math.min(end, Math.max(begin, offset));
        if (track.getDirection().equals(Direction.FORWARD)) return new TrackRangeView(begin, offset, track);
        else return new TrackRangeView(offset, end, track);
    }
    ...
    private double convertPosition(double position) {
        if (track.getDirection() == Direction.FORWARD) return position - begin;
        return end - position;
    }
    ...
    private Range<Double> convertRange(Range<Double> range) {
        ...
        if (rangeStart != rangeEnd) return Range.open(rangeStart, rangeEnd);
        else return null;
    }
    ...
    public <T> ImmutableRangeMap<Double, T> convertMap(RangeMap<Double, T> map) {
        if (getLength() == 0) return ImmutableRangeMap.of();
        ...
        for (var entry : subMap.asMapOfRanges().entrySet()) {
            ...
            if (newRange != null) builder.put(newRange, entry.getValue());
        }
    ...
    /**
     * Returns the track location given an offset over a list of track ranges. Throw a runtime
     * exception if the offset is not contains in the track ranges
     */
    public static TrackLocation getLocationFromList(List<TrackRangeView> ranges, double offset) {
        double iterOffset = offset;
        for (var range : ranges) {
            if (range.getLength() >= iterOffset) return range.offsetLocation(iterOffset);
