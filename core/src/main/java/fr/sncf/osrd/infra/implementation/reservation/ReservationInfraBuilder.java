...
public class ReservationInfraBuilder {
     ...
     * @return a new OSRDError instance
     */
    public static OSRDError newInvalidInfraMissingDetectorsError(String routeID, int nbDetectors) {
    ...
    public static ReservationInfra fromDiTrackInfra(
            RJSInfra rjsInfra, DiTrackInfra diTrackInfra, DiagnosticRecorder diagnosticRecorder) {
    ...
    private ReservationInfra build() {
        var reservationSections = DetectionSectionBuilder.build(diTrackInfra);
    ...
    private static ImmutableMultimap<DiTrackEdge, ReservationInfra.RouteEntry> makeRouteOnEdgeMap(
        ...
        for (var route : routes) {
            ...
            for (var range : route.getTrackRanges()) {
                // skip switch branch
                if (range.track.getEdge() instanceof SwitchBranch) continue;
                double rangeOffset; // Offset from the start of the track to the start of the range
                if (range.track.getDirection().equals(FORWARD)) rangeOffset = range.begin;
                else rangeOffset = range.track.getEdge().getLength() - range.end;
    ...
    private ImmutableMap<String, ReservationRoute> makeReservationRouteMap(
        ...
        for (var route : routeGraph.edges()) res.put(route.getID(), route);
        return res.build();
    }
    ...
    private ImmutableNetwork<DiDetector, ReservationRoute> makeRouteGraph(
        ...
        var networkBuilder =
                NetworkBuilder.directed().allowsParallelEdges(true).<DiDetector, ReservationRoute>immutable();
        var routesPerSection = new IdentityHashMap<DetectionSection, ImmutableSet.Builder<ReservationRoute>>();
        for (var section : reservationSections) routesPerSection.put(section, new ImmutableSet.Builder<>());
        var routes = new ArrayList<ReservationRouteImpl>();
        for (var rjsRoute : rjsInfra.routes) {
            var trackRanges = makeTrackRanges(rjsRoute);
            var length =
                    trackRanges.stream().mapToDouble(TrackRangeView::getLength).sum();
            var detectors = detectorsOnRoute(trackRanges);
            var sections = sectionsOnRoute(detectors);
            var route = new ReservationRouteImpl(
                    detectors,
                    releasePoints(rjsRoute),
                    rjsRoute.id,
                    trackRanges,
                    isRouteControlled(rjsRoute),
                    length,
                    sections);
            validateRoute(route, rjsRoute);
            routes.add(route);
            for (var section : sections) routesPerSection.get(section).add(route);
        }
    ...
    private void validateRoute(ReservationRouteImpl route, RJSRoute rjsRoute) {
        if (route.getDetectorPath().size() < 2)
            diagnosticRecorder.register(newInvalidInfraMissingDetectorsError(
                    rjsRoute.id, route.getDetectorPath().size()));

        var detectorIDs =
                route.getDetectorPath().stream().map(x -> x.detector().getID()).collect(Collectors.toSet());
        for (var releasePoint : route.getReleasePoints()) {
            if (!detectorIDs.contains(releasePoint.getID()))
                diagnosticRecorder.register(new Warning(
                        String.format("Release detector for route %s is not part of the route", rjsRoute.id)));
        }
        ...
        if (rjsRoute.entryPoint != null) {
            ...
            if (!entryDetector.getID().equals(rjsRoute.entryPoint.id.id))
                diagnosticRecorder.register(new Warning(String.format(
                        ...
                        rjsRoute.id, rjsRoute.entryPoint.id.id, detectorIDs)));
            ...
            if (!entryPointLocation.equalsWithTolerance(routeStartLocation, 1e-8)) {
                diagnosticRecorder.register(new Warning(String.format(
                        ...
                        rjsRoute.id, entryPointLocation, routeStartLocation)));
            }
        }
        if (rjsRoute.exitPoint != null) {
            ...
            if (!lastDetector.detector().getID().equals(rjsRoute.exitPoint.id.id))
                diagnosticRecorder.register(new Warning(String.format(
                        "Exit point for route %s don't match the last detector on the route "
                                + "(expected = %s, detector list = %s)",
                        rjsRoute.id, rjsRoute.exitPoint.id.id, detectorIDs)));
            var exitPointLocation = new TrackLocation(exitDetector.getTrackSection(), exitDetector.getOffset());
            var lastRouteRange =
                    route.getTrackRanges().get(route.getTrackRanges().size() - 1);
            ...
            if (!exitPointLocation.equalsWithTolerance(routeEndLocation, 1e-8)) {
                diagnosticRecorder.register(new Warning(String.format(
                        ...
                        rjsRoute.id, exitPointLocation, routeEndLocation)));
            }
        }
    }
    ...
    private ImmutableList<TrackRangeView> makeTrackRanges(RJSRoute rjsRoute) {
        ...
        while (edge.getEdge() != exitEdge) {
            ...
            if (neighbors.isEmpty()) throw newDiscontinuousRouteError(rjsRoute.id);
