...
class DeprecatedPathfinding<NodeT : Any, EdgeT : Any, OffsetType>(
    ...
    data class Range(val start: Offset<Block>, val end: Offset<Block>)

    /** Step priority queue */
    private val queue = PriorityQueue<Step<EdgeT>>()

    /** Function to call to get edge length */
    private var edgeToLength: EdgeToLength<EdgeT, Block>? = null
     ...
     */
    private var estimateRemainingDistance: List<AStarHeuristic<EdgeT, Block>>? = ArrayList()
    ...
    fun setRemainingDistanceEstimator(
        f: List<AStarHeuristic<EdgeT, Block>>?
    ): DeprecatedPathfinding<NodeT, EdgeT, OffsetType> {
            ...
            if (step.prev == null || step.nReachedTargets == step.prev.nReachedTargets)
                for (target in targetsOnEdges[step.nReachedTargets].apply(step.range.edge)) {
                    if (step.range.start <= target.offset) {
                        if (
                            (step.targets.lastOrNull()?.offset ?: Offset<Block>(0.meters)) >
                                target.offset
                        ) {
