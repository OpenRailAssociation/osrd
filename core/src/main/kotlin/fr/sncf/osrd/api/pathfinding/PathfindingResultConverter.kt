...
private fun chunksOnBlocks(
    blockInfra: BlockInfra,
    blockIds: List<BlockId>
): DirStaticIdxList<TrackChunk> {
    val res = mutableDirStaticIdxArrayListOf<TrackChunk>()
    for (block in blockIds) for (chunk in blockInfra.getTrackChunksFromBlock(block)) res.add(chunk)
    return res
}
...
fun chunksToRoutes(
    ...
    while (chunkStartIndex < pathChunks.size) {
        ...
        ) chunkStartIndex++ // Increase the index in the chunk path, for as
        // long as it is covered by the route
    }
    return res
}
...
private fun findRoute(
    ...
    for (routeId in sortedRoutes) if (
        routeMatchPath(infra, blockInfra, chunks, startIndex, routeMustIncludeStart, routeId)
    )
        return routeId
...
private fun routeHasBlockPath(
    ...
    val blockPaths = recoverBlocks(infra, blockInfra, routeIds, null)
