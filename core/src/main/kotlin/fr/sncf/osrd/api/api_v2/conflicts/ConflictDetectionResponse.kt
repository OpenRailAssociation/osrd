...
class ConflictDetectionResponse(
    /**
     * List of all detected conflicts, if any. Conflicts are grouped between each set of conflicting
     * train/work schedule IDs. (i.e. there's only one conflict entry between trains (A, B), but
     * there may be a different one for trains (A, B), or (B, C, D)).
     */
    val conflicts: Collection<Conflict>,
)

/**
 * One conflict between a set of trains. If a given set of train is conflicting over a continuous
 * time range, only one conflict is returned (with a longer requirement list).
 */
class Conflict(
 ...
 * [trains / work schedule] in the given time range. This time range is the *union* and *not the
 * intersection* of the time during which they are used by at least one train. i.e. if a train uses
 * the resource from 10:00 to 11:00 and another one uses it from 10:30 to 11:30, the resulting range
