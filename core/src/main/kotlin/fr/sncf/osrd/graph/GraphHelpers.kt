...
fun nodeFromEdgeEndpoint(
    ...
    edge: TrackEdge, endpoint: EdgeEndpoint
): TrackNode {
    val nodes = undirectedGraph.incidentNodes(edge)
    return if (endpoint == EdgeEndpoint.BEGIN)
        nodes.nodeU()
    else
        nodes.nodeV()
}
...
fun getCommonNodes(
    undirectedGraph: ImmutableNetwork<TrackNode, TrackEdge>,
    edge1: TrackEdge, edge2: TrackEdge
): Collection<TrackNode?> {
    val intersection = Sets.intersection(
