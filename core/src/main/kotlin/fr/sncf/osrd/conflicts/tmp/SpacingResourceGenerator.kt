...
import fr.sncf.osrd.conflicts.IncrementalRequirementCallbacks
import fr.sncf.osrd.conflicts.SpacingRequirement
import fr.sncf.osrd.conflicts.TrainPathSignal
...
class SpacingRequirementAutomaton(
    ...
    private fun registerPathExtension() {
        // TODO path migration: double check this
    ...
    private fun setupInitialRequirements() {
        ...
        var startZone = -1
        val startingPoint = Offset.zero<TrainPath>()
        for (i in 0 until incrementalPath.zones.size) {
            val zonePathStartOffset = incrementalPath.zones[i].pathBegin
            val zonePathEndOffset = incrementalPath.zones[i].pathEnd
