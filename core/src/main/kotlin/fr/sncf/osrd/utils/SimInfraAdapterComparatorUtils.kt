    ...
    for (trackSectionId in compare { trackSections }) {
        ...
        for (chunkId in compare { getTrackSectionChunks(trackSectionId) }) {
            ...
            for (dir in Direction.entries) {
                ...
                compare { getTrackChunkSpeedSections(dirChunkId, null, null) }
                for (routeId in compare { getRoutesOnTrackChunk(dirChunkId) }) {
                    compare {
                        getTrackChunkSpeedSections(dirChunkId, null, left.getRouteName(routeId))
                    }
                }
            }
        }
