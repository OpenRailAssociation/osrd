...
private class InfraExplorerImpl(
    ...
    override fun getCurrentEdgePathProperties(offset: Offset<Block>, length: Distance?): PathProperties {
        return PathPropertiesView(
            blockToPathProperties[getCurrentBlock()]!!,
            offset.cast(),
            if (length == null)
                Offset(blockInfra.getBlockLength(getCurrentBlock()).distance)
            else
                offset.plus(length).cast()
        )
    }
