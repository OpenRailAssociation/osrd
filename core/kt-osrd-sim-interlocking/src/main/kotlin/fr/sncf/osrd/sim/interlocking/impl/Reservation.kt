 ...
 */
internal data class ZoneStateImpl(
    override val reservations: ArenaMap<ZoneReservation, ZoneReservation>
) : ZoneState
...
internal class ReservationSimImpl(
    ...
    /**
     * This job starts when the reservation is confirmed, and makes the reservation go occupied,
     * then pending release.
     */
    ...
    override fun preReserve(zone: ZoneId, zonePath: ZonePathId, train: TrainId): ZoneReservationId {
        if (!locks[zone.index].isLocked) throw OSRDError(ErrorType.ActionLockRequired)
        ...
        states[zone.index].update { prevState ->
            ...
            ZoneStateImpl(
                prevState.reservations.update { reservationIndex = it.allocate(newReservation) }
            )
        }
    ...
    override fun release(zone: ZoneId, reservation: ZoneReservationId) {
        states[zone.index].update { prevState ->
            ZoneStateImpl(
                prevState.reservations.update { arena ->
