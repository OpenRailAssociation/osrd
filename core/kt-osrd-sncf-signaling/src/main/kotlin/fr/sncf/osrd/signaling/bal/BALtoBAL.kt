...
object BALtoBAL : SignalDriver {
    ...
    private fun cascadePrimaryAspect(
        aspect: String,
        parameters: SigParameters,
        nextDistanceToStop: Distance,
    ): String {
        return when (aspect) {
            "VL" -> "VL"
            "A" ->
            ...
            "(A)" ->
                if (
                    parameters.getFlag("jaune_cli") &&
                        nextDistanceToStop > 0.meters &&
                        nextDistanceToStop < 500.meters
                ) {
                    "(A)"
                } else {
                    "VL"
            ...
            when (maView!!.protectionStatus) {
                ...
                CLEAR -> {
                    if (!maView.hasNextSignal) {
                        value("aspect", "A")
                        // TODO: maybe we should include the distance to the bufferstop in the
                        // MAView as the next signal distance?
                        value("distance_to_stop", -1)
                    } else {
                        val nextSignalState = maView.nextSignalState
                        val nextAspect = nextSignalState.getEnum("aspect")
                        val nextDistanceToStop =
                            nextSignalState.getInt("distance_to_stop").millimeters
                        value(
                            "aspect",
                            cascadePrimaryAspect(nextAspect, parameters, nextDistanceToStop),
                        )
                        if (nextDistanceToStop == (-1).millimeters) value("distance_to_stop", -1)
                        else {
                            val distanceToNextSignal = maView.distanceToNextSignal!!
                            // FIXME: really, we should be able to encode longs, but this should
                            // work to demonstrate the concept
                            value(
                                "distance_to_stop",
                                (distanceToNextSignal + nextDistanceToStop).millimeters.toInt(),
                            )
                        }
                    }
                }
