...
public class UnionFind {
    ...
    public UnionFind(int nbGroup) {
        this();
        for (int i = 0; i < nbGroup; i++) this.parents.add(-1);
    }
    ...
    public int findRoot(int group) {
        while (parents.get(group) != -1) group = parents.get(group);
        return group;
    }
    ...
    public void union(int groupA, int groupB) {
        ...
        if (groupA < groupB) parents.set(groupB, groupA);
        else parents.set(groupA, groupB);
    }

    private ArrayList<Integer> resolve() {
        var res = new ArrayList<Integer>();
        // resolve the chain of merged components
        for (int i = 0; i < parents.size(); i++) res.add(findRoot(i));
        return res;
    }
