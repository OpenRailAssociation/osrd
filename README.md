...
First, to enable the flake commands, which are still experimental, you need to add this line to the config file `~/.config/nix/nix.conf` (create it if it doesn't exist):
```ini
