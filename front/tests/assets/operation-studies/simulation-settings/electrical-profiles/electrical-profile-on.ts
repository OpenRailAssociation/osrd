import type { TimesStopsTableRow } from '../../../../utils/times-stops-table-types';

export const electricalProfileOnData: TimesStopsTableRow[] = [
  {
    index: 1,
    status: 'success-schedule',
    stationName: 'West_station',
    stationCh: 'BV',
    trackName: 'V2',
    requestedArrival: '11:22:40',
    calculatedArrival: '11:22:40',
    stopTime: '',
    requestedDeparture: '',
    calculatedDeparture: '•',
    signalReceptionClosed: false,
    shortSlipDistance: false,
    powerRestriction: '',
    margin: {
      requestedTheoretical: '0%',
      computedTheoretical: '00m00s',
      real: '00m00s',
      difference: '00m00s',
    },
    timeFromAboveWaypoint: '',
    totalArrivalTime: '',
    referenceBaseArrival: '11:22:40',
  },
  {
    index: 2,
    status: '',
    stationName: 'Mid_West_station',
    stationCh: 'BV',
    trackName: 'V1',
    requestedArrival: '',
    calculatedArrival: '11:29:28',
    stopTime: '',
    requestedDeparture: '',
    calculatedDeparture: '•',
    signalReceptionClosed: false,
    shortSlipDistance: false,
    powerRestriction: '',
    margin: { requestedTheoretical: '', computedTheoretical: '', real: '', difference: '' },
    timeFromAboveWaypoint: '',
    totalArrivalTime: '',
    referenceBaseArrival: '11:29:28',
  },
  {
    index: 3,
    status: '',
    stationName: 'Mid_East_station',
    stationCh: 'BV',
    trackName: 'V1',
    requestedArrival: '',
    calculatedArrival: '11:34:43',
    stopTime: '00h02m04s',
    requestedDeparture: '',
    calculatedDeparture: '11:36:47',
    signalReceptionClosed: false,
    shortSlipDistance: false,
    powerRestriction: '',
    margin: { requestedTheoretical: '', computedTheoretical: '', real: '', difference: '' },
    timeFromAboveWaypoint: '',
    totalArrivalTime: '',
    referenceBaseArrival: '11:34:43',
  },
  {
    index: 4,
    status: '',
    stationName: 'South_East_station',
    stationCh: 'BV',
    trackName: 'V1',
    requestedArrival: '',
    calculatedArrival: '11:47:46',
    stopTime: '00h00m00s',
    requestedDeparture: '',
    calculatedDeparture: '11:47:46',
    signalReceptionClosed: false,
    shortSlipDistance: false,
    powerRestriction: '',
    margin: { requestedTheoretical: '', computedTheoretical: '', real: '', difference: '' },
    timeFromAboveWaypoint: '',
    totalArrivalTime: '',
    referenceBaseArrival: '11:47:46',
  },
];
