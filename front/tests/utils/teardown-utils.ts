...
import { getApiRequest, deleteApiRequest } from './api-setup';
...
export async function deleteProject(projectName: string): Promise<void> {
  ...
  const project = projects.results.find((p: Project) => p.name === projectName);
...
export async function deleteStudy(projectId: number, studyName: string): Promise<void> {
  const studies = await getApiRequest(`/api/projects/${projectId}/studies/`);
  const study = studies.results.find((s: Study) => s.name === studyName);
...
export async function deleteScenario(
  ...
  const scenarios = await getApiRequest(`/api/projects/${projectId}/studies/${studyId}/scenarios`);
  const scenario = scenarios.results.find((s: Scenario) => s.name === scenarioName);
