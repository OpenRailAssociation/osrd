...
test.describe('Rollingstock editor page', () => {
  test('should correctly create a new rolling stock (enabled)', async ({ page }) => {
    const playwrightCommonPage = new PlaywrightCommonPage(page);
    const playwrightRollingstockEditorPage = new PlaywrightRollingstockEditorPage(page);

    await page.goto('/rolling-stock-editor');

    await playwrightRollingstockEditorPage.clickOnNewRollingstockButton();

    await playwrightRollingstockEditorPage.fillAndCheckInputById('name', 'Test name');
    ...
    await playwrightCommonPage.checkToastSNCFTitle(/Formulaire incomplet/);
    ...
    await expect(tractionModesSelector.getByTitle('1000')).toBeVisible();
    ...
    await powerRestrictionModal.getByTitle('C0').click();

    await expect(powerRestrictionSelector.getByTitle('C0')).toBeVisible();
    ...
    await playwrightRollingstockEditorPage.getRollingStockSearchInput.fill('Test name');

    expect(await playwrightRollingstockEditorPage.page.locator('.rollingstock-card').count()).toBe(
      1
    );
  ...
  test('should correctly duplicate and modify a rolling stock (enabled)', async ({ page }) => {
    ...
    await playwrightRollingstockEditorPage.page
      .getByTestId(`rollingstock-rollingstock_copied`)
