...
import { getTruncatedToSecondSchedule } from './utils';
...
export const cascadeArrivals = ({
  ...
  for (const { item, pathIndex } of scheduledItems) {
    const stop = item.stop_for ? getTruncatedToSecondSchedule(item.stop_for) : Duration.zero;
    if (!item.arrival) {
      lastOffset = lastOffset.add(stop);
      continue;
    }
    const arrival = getTruncatedToSecondSchedule(item.arrival);
