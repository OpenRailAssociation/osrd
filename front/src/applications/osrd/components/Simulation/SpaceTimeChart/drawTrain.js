...
export default function drawTrain(
  chart, dispatch, dataSimulation, isPathSelected, isSelected, keyValues, allowancesSettings,
  ...
  if (direction) {
    if (allowancesSettings[dataSimulation.id].baseBlocks
    ...
    if (dataSimulation.eco_routeEndOccupancy
      && allowancesSettings[dataSimulation.id].ecoBlocks) {
      dataSimulation.eco_areaBlock.forEach(
        (dataSimulationEcoAreaBlockSection) => drawArea(
          chart, `${isSelected && 'selected'} area eco`, dataSimulationEcoAreaBlockSection,
          groupID, 'curveStepAfter', keyValues, rotate,
        ),
      );
      dataSimulation.eco_routeEndOccupancy.forEach(
        (ecoRouteEndOccupancySection) => drawCurve(
          chart, `${isSelected && 'selected'} end-block`, ecoRouteEndOccupancySection, groupID,
          'curveLinear', keyValues, 'eco_routeEndOccupancy', rotate, isSelected,
        ),
      );
      dataSimulation.eco_routeBeginOccupancy.forEach(
        (ecoRouteBeginOccupancySection) => drawCurve(
          chart, `${isSelected && 'selected'} start-block`, ecoRouteBeginOccupancySection, groupID,
  ...
  if (dataSimulation.allowances_headPosition && allowancesSettings[dataSimulation.id].allowances) {
    dataSimulation.allowances_headPosition.forEach((tailPositionSection) => drawCurve(
      chart, `${isSelected && 'selected'} head allowances`, tailPositionSection, groupID,
      'curveLinear', keyValues, 'allowances_headPosition', rotate, isSelected,
    ));
  }
