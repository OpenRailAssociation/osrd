...
const drawRect = (
  ...
  drawZone.append('rect')
    .attr('class', `rect zoomable ${classes}`)
    .datum(dataSimulation)
    .attr('fill', dataSimulation.color)
    .attr('stroke-width', 1)
    .attr('stroke', dataSimulation.color)
    .attr('x', chart.x(rotate ? dataSimulation[`${keyValues[1]}_start`] : dataSimulation[`${keyValues[0]}_start`]))
    .attr('y', chart.y(rotate ? dataSimulation[`${keyValues[0]}_start`] : dataSimulation[`${keyValues[1]}_start`]) - height*-1)
    .attr('width', width)
