...
import { TimetableWithSchedulesDetails, osrdEditoastApi } from 'common/api/osrdEditoastApi';

/**
 * Recover the time table for all the trains
 */

export default async function getTimetable(timetable: TimetableWithSchedulesDetails) {
  ...
  try {
    ...
    const response = osrdEditoastApi.endpoints.getTrainScheduleResults.select({
      pathId: 1,
      trainIds: '1',
    });
