...
export default class OsrdNgeSync {
  ...
  /**
   * We keep a dictionnary of id/key to be able to find a node its id
   */
  ...
  /**
   * Load & index the data of the tran schedule for the given scenario
   */
    ...
    this.trainSchedules
      ...
      .forEach((pathItem, index) => {
        ...
        if (!this.getNodeByKey(key)) {
          ...
          // index node
          this.indexNode(macroNodes);
        }
      });

    // Enhance nodes by calling the search API
    const searchResults = await this.searchPathItems();
    searchResults.forEach((searchResult) => {
      const macroNodes = {
        full_name: searchResult.name,
        position_x: searchResult.geographic.coordinates[0],
        position_y: searchResult.geographic.coordinates[1],
        trigram: searchResult.trigram + (searchResult.ch ? `/${searchResult.ch}` : ''),
      };
      OsrdNgeSync.getPathKeys(searchResult).forEach((key) => {
        if (this.getNodeByKey(key)) {
          this.updateNodeData(key, macroNodes);
        }
      });
    });

    // Load saved node and index them by path key
    const savedNodes = await this.apiGetSavedNodes();
    savedNodes.forEach((n) => {
      this.indexNode(n, true);
    });
   ...
   * Ex: one key is trigram and other is uic (with the same trigram), we should keep trigram
   * What we do :
   ...
   */
  private updateNodeData(key: string, data: Partial<Omit<MacroNodeResponse, 'path_item_key'>>) {
    const indexedNode = this.getNodeByKey(key);
    if (indexedNode) {
      this.indexNode({ ...indexedNode.node, ...data }, indexedNode.saved);
    }
  }
  ...
  private applyLayout() {
    ...
    if (graph.nodes.some((n) => !n.fixed)) {
      const layout = new cola.Layout()
        ...
        .nodes(graph.nodes)
        .links(graph.edges)
        .start(10, 15, 20, 0, false, false);

      layout.nodes().forEach((n) => {
        const node = n as cola.Node & { key: string };
        this.updateNodeData(node.key, {
          position_x: node.x,
          position_y: node.y,
        });
      });
  ...
  private toGraph(): {
    ...
    const nodes = uniqBy(this.trainSchedules.map((ts) => ts.path).flat(), (e) => e.id).map(
      (pathItem) => {
        const key = OsrdNgeSync.getPathKey(pathItem);
    ...
    this.trainSchedules.forEach((ts) => {
      ...
      ts.path.forEach((item) => {
        ...
        if (prevInNodeArrayIndex) {
          edges.push({ source: prevInNodeArrayIndex, target: targetNodeIndexInNodeArray });
        }
  ...
  private getNgeTrainrunSectionsWithNodes() {
      ...
      .map((trainSchedule) => {
        ...
        return pathNodeKeys.slice(0, -1).map((sourceNodeKey, i) => {
          ...
          if (!ngeNodesByPathKey[sourceNodeKey]) {
            ngeNodesByPathKey[sourceNodeKey] = this.castNodeToNge(
              this.getNodeByKey(sourceNodeKey)!.node
            );
          }
    ...
    // Check that all saved nodes are in the list, so they are displayed on NGE
    Object.values(this.nodesByPathKey).forEach((indexNode) => {
      if (typeof indexNode !== 'string' && !ngeNodesByPathKey[indexNode.node.path_item_key]) {
        ngeNodesByPathKey[indexNode.node.path_item_key] = this.castNodeToNge(indexNode.node);
      }
    });
   ...
   */
  private async searchPathItems(): Promise<SearchResultItemOperationalPoint[]> {
    const searchResults: SearchResultItemOperationalPoint[] = [];
    const searchPayload = this.getSearchPathItemsPayload();
    if (searchPayload) {
      const pageSize = 100;
      let done = false;
   ...
   * trigrams and IDs.
   */
  private getSearchPathItemsPayload(): SearchPayload | null {
   ...
   */
  private castNodeToNge(node: MacroNodeResponse): NetzgrafikDto['nodes'][0] {
  ...
  private async apiDeleteNode(node: MacroNodeResponse) {
    try {
      ...
      this.deleteNode(node.path_item_key);
    } catch (e) {
      console.error(e);
    }
  }
  ...
  /**
   * Given an path step, returns its key
   */
  ...
  /**
   * Given a search result item, returns all possible path key, ordered by importance.
   */
