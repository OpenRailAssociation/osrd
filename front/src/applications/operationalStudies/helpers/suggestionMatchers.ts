import { norm, normalizePhrase, splitTokens, toUpper } from 'utils/strings';
...
export const chStarts = (s: OperationalPointSuggestion, tokenUpper: string) =>
  s.chList.some((secondaryCode) => toUpper(secondaryCode.code).startsWith(tokenUpper));
export const chIncludes = (s: OperationalPointSuggestion, tokenUpper: string) =>
  s.chList.some((secondaryCode) => toUpper(secondaryCode.code).includes(tokenUpper));
...
export const hasChPrefix = (s: OperationalPointSuggestion, chTokenUpper: string) =>
  s.chList.some((secondaryCode) => toUpper(secondaryCode.code).startsWith(chTokenUpper));
