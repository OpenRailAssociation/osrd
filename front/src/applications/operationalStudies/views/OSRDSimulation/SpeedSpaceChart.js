...
import createCurveCurve from 'applications/operationalStudies/components/Simulation/SpeedSpaceChart/createCurveCurve';
import createSlopeCurve from 'applications/operationalStudies/components/Simulation/SpeedSpaceChart/createSlopeCurve';
...
import OSRDSpeedSpaceSettings from 'applications/operationalStudies/components/Simulation/SpeedSpaceSettings/withOSRDData';
...
export default function SpeedSpaceChart(props) {
  ...
  const createChart = () => {
    ...
    const width = parseInt(d3.select(`#container-${CHART_ID}`)?.style('width') || 250, 10);
    if (resetChart) {
