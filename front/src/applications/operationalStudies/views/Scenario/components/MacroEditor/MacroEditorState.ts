...
export default class MacroEditorState {
  ...
  static getPathKey(item: PathItemLocation): string {
    ...
    if (item.operational_point.type === 'domestic') {
      return `domestic:${this.encodeDomesticReference(item.operational_point)}`;
    }
   ...
   */
  static encodeDomesticReference(opRef: {
    main_code: string;
    secondary_code?: string | null;
    country_code?: string | null;
  }): string {
    const secondary_code = opRef.secondary_code ? `/${opRef.secondary_code}` : '';
    const country_code =
      !opRef.country_code || opRef.country_code === '??' ? '' : `${opRef.country_code}-`;
    return `${country_code}${opRef.main_code}${secondary_code}`;
  }
