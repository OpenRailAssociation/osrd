import type { LayerProps } from 'react-map-gl/maplibre';

import type { MapStyle } from 'reducers/globalMap/types';

export default function getOSMDarkStyle(_mapStyle: MapStyle): LayerProps[] {
  return [
    {
      id: 'landuse-residential',
      type: 'fill',
      source: 'openmaptiles',
      'source-layer': 'landuse',
      filter: ['all', ['==', '$type', 'Polygon'], ['==', 'class', 'residential']],
      layout: {
        visibility: 'visible',
      },
      paint: {
        'fill-color': '#120d1c',
        'fill-opacity': 0.7,
      },
    },
    {
      id: 'water',
      type: 'fill',
      source: 'openmaptiles',
      'source-layer': 'water',
      filter: ['==', '$type', 'Polygon'],
      layout: {
        visibility: 'visible',
      },
      paint: {
        'fill-color': '#151e2e',
      },
    },
    {
      id: 'waterway',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'waterway',
      filter: ['==', '$type', 'LineString'],
      layout: {
        visibility: 'visible',
      },
      paint: {
        'line-color': '#151e2e',
        'line-width': ['interpolate', ['exponential', 1.4], ['zoom'], 8, 1, 20, 8],
        'line-opacity': 1,
      },
    },
    {
      id: 'building',
      type: 'fill',
      source: 'openmaptiles',
      'source-layer': 'building',
      layout: {
        visibility: 'visible',
      },
      paint: {
        'fill-color': '#111832',
        'fill-outline-color': '#0a1025',
        'fill-opacity': ['interpolate', ['exponential', 1], ['zoom'], 13, 0, 15, 1],
        'fill-antialias': true,
      },
    },
    {
      id: 'housenumber',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'housenumber',
      minzoom: 17,
      filter: ['==', '$type', 'Point'],
      layout: {
        'text-field': '{housenumber}',
        'text-size': 10,
        'text-font': ['IBMPlexSerif-Regular'],
      },
      paint: {
        'text-color': '#e0e1dd',
      },
    },
    {
      id: 'ruisseaux',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'waterway',
      filter: ['all', ['==', 'class', 'stream']],
      layout: {
        'text-field': '{name}',
        'text-font': ['IBMPlexSerif-Regular'],
        'symbol-placement': 'line',
        'text-size': 10,
      },
      paint: {
        'text-color': 'rgba(94, 136, 185, 1)',
        'text-halo-color': 'rgba(255, 255, 255, 1)',
        'text-halo-width': 0,
        'text-halo-blur': 0,
      },
    },
    {
      id: 'rivieres',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'waterway',
      minzoom: 12,
      maxzoom: 24,
      filter: ['all', ['==', 'class', 'river']],
      layout: {
        'text-field': '{name}',
        'text-font': ['IBMPlexSerif-Bold'],
        'symbol-placement': 'line-center',
        'text-size': 12,
        'text-pitch-alignment': 'auto',
        'text-rotation-alignment': 'auto',
        'symbol-avoid-edges': false,
      },
      paint: {
        'text-color': 'rgba(94, 136, 185, 1)',
        'text-halo-color': 'rgba(255, 255, 255, 1)',
        'text-halo-width': 0,
        'text-halo-blur': 0,
      },
    },
    {
      id: 'road_path',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: ['all', ['==', '$type', 'LineString'], ['in', 'class', 'path', 'track']],
      layout: {
        'line-cap': 'square',
        'line-join': 'bevel',
        visibility: 'visible',
      },
      paint: {
        'line-color': '#33455c',
        'line-dasharray': [1, 1],
        'line-width': ['interpolate', ['exponential', 1.55], ['zoom'], 4, 1, 20, 3],
      },
    },
    {
      id: 'road_minor',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: ['all', ['==', '$type', 'LineString'], ['in', 'class', 'minor', 'service']],
      layout: {
        'line-cap': 'round',
        'line-join': 'round',
        visibility: 'visible',
      },
      paint: {
        'line-color': '#1f2b3c',
        'line-width': ['interpolate', ['exponential', 1.55], ['zoom'], 4, 0.25, 20, 15],
      },
    },
    {
      id: 'tunnel_minor',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: [
        'all',
        ['==', '$type', 'LineString'],
        ['==', 'brunnel', 'tunnel'],
        ['==', 'class', 'minor_road'],
      ],
      layout: {
        'line-cap': 'butt',
        'line-join': 'miter',
      },
      paint: {
        'line-color': '#efefef',
        'line-width': ['interpolate', ['exponential', 1.55], ['zoom'], 4, 0.25, 20, 30],
        'line-dasharray': [0.36, 0.18],
      },
    },
    {
      id: 'tunnel_major',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: [
        'all',
        ['==', '$type', 'LineString'],
        ['==', 'brunnel', 'tunnel'],
        ['in', 'class', 'primary', 'secondary', 'tertiary', 'trunk'],
      ],
      layout: {
        'line-cap': 'butt',
        'line-join': 'miter',
      },
      paint: {
        'line-color': 'rgba(83, 83, 83, 0.99)',
        'line-width': ['interpolate', ['exponential', 1.4], ['zoom'], 6, 0.25, 20, 30],
        'line-dasharray': [0.28, 0.14],
      },
    },
    {
      id: 'aeroway-area',
      type: 'fill',
      source: 'openmaptiles',
      'source-layer': 'aeroway',
      minzoom: 4,
      filter: ['all'],
      layout: {
        visibility: 'visible',
      },
      paint: {
        'fill-opacity': 1,
        'fill-color': '#1b263b',
      },
    },
    {
      id: 'aeroway-taxiway',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'aeroway',
      minzoom: 12,
      filter: ['all', ['in', 'class', 'taxiway'], ['==', '$type', 'LineString']],
      layout: {
        'line-cap': 'round',
        'line-join': 'round',
        visibility: 'visible',
      },
      paint: {
        'line-color': 'rgba(255, 255, 255, 1)',
        'line-width': ['interpolate', ['exponential', 1.5], ['zoom'], 12, 1, 17, 10],
        'line-opacity': 0.1,
      },
    },
    {
      id: 'aeroway-runway',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'aeroway',
      minzoom: 4,
      filter: ['all', ['in', 'class', 'runway'], ['==', '$type', 'LineString']],
      layout: {
        'line-cap': 'butt',
        'line-join': 'round',
        visibility: 'visible',
      },
      paint: {
        'line-color': '#fff',
        'line-width': ['interpolate', ['exponential', 1.5], ['zoom'], 11, 0, 17, 50],
        'line-opacity': 0.2,
      },
    },
    {
      id: 'road_trunk_primary',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: ['all', ['==', '$type', 'LineString'], ['in', 'class', 'trunk', 'primary']],
      layout: {
        'line-cap': 'round',
        'line-join': 'round',
      },
      paint: {
        'line-color': '#233346',
        'line-width': ['interpolate', ['exponential', 1.4], ['zoom'], 6, 0.5, 20, 30],
      },
    },
    {
      id: 'road_secondary_tertiary',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: ['all', ['==', '$type', 'LineString'], ['in', 'class', 'secondary', 'tertiary']],
      layout: {
        'line-cap': 'round',
        'line-join': 'round',
        visibility: 'visible',
      },
      paint: {
        'line-color': '#27364a',
        'line-width': ['interpolate', ['exponential', 1.4], ['zoom'], 6, 0.05, 20, 10],
      },
    },
    {
      id: 'road_major_motorway',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: ['all', ['==', '$type', 'LineString'], ['==', 'class', 'motorway']],
      layout: {
        'line-cap': 'round',
        'line-join': 'round',
      },
      paint: {
        'line-color': '#232c3d',
        'line-width': ['interpolate', ['exponential', 1.4], ['zoom'], 8, 1, 16, 10],
        'line-offset': 0,
      },
    },
    {
      id: 'batiments_3d',
      source: 'openmaptiles',
      'source-layer': 'building',
      type: 'fill-extrusion',
      minzoom: 15,
      paint: {
        'fill-extrusion-color': '#111832',
        'fill-extrusion-height': [
          'interpolate',
          ['linear'],
          ['zoom'],
          15,
          0,
          15.05,
          ['get', 'render_height'],
        ],
        'fill-extrusion-base': [
          'interpolate',
          ['linear'],
          ['zoom'],
          15,
          0,
          15.05,
          ['get', 'render_min_height'],
        ],
        'fill-extrusion-opacity': 1,
      },
    },
    {
      id: 'admin_country',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'boundary',
      filter: ['all', ['<=', 'admin_level', 2], ['==', '$type', 'LineString']],
      layout: {
        'line-cap': 'round',
        'line-join': 'round',
      },
      paint: {
        'line-color': 'rgba(146, 146, 146, 1)',
        'line-width': ['interpolate', ['exponential', 1.3], ['zoom'], 3, 5, 22, 15],
      },
    },
    {
      id: 'poi_label',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'poi',
      minzoom: 5,
      filter: ['all', ['==', '$type', 'Point'], ['==', 'rank', 1], ['!=', 'class', 'railway']],
      layout: {
        'text-size': 12,
        'text-font': ['IBMPlexSerif-SemiBold'],
        visibility: 'visible',
        'text-offset': [0, 0.5],
        'icon-size': 1,
        'text-anchor': 'top',
        'text-field': '{name:latin}\n{name:nonlatin}',
        'text-max-width': 8,
        'text-transform': 'none',
      },
      paint: {
        'text-color': '#475a71',
        'text-halo-color': '#0b011d',
        'text-halo-width': 1,
      },
    },
    {
      id: 'road_major_label',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'transportation_name',
      filter: ['all', ['==', '$type', 'LineString']],
      layout: {
        'symbol-placement': 'line',
        'text-field': '{name:latin} {name:nonlatin} {ref}',
        'text-font': ['IBMPlexSerif-Regular'],
        'text-transform': 'uppercase',
        'text-letter-spacing': 0,
        'text-size': ['interpolate', ['exponential', 1.4], ['zoom'], 10, 8, 20, 13],
        'text-rotation-alignment': 'map',
        'text-keep-upright': true,
        visibility: 'visible',
      },
      paint: {
        'text-color': '#95b5da',
      },
    },
    {
      id: 'place_label_other',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'place',
      minzoom: 8,
      filter: ['all', ['==', '$type', 'Point'], ['!=', 'class', 'city']],
      layout: {
        'text-field': '{name:latin}\n{name:nonlatin}',
        'text-font': ['IBMPlexSerif-Medium'],
        'text-max-width': 6,
        'text-size': ['interpolate', ['exponential', 1], ['zoom'], 6, 8, 12, 11, 14, 15],
        visibility: 'visible',
        'text-anchor': 'center',
        'text-justify': 'center',
        'text-transform': 'uppercase',
        'icon-size': 1,
        'text-ignore-placement': false,
        'text-allow-overlap': false,
      },
      paint: {
        'text-color': '#3c4b5e',
      },
    },
    {
      id: 'country_label',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'place',
      maxzoom: 12,
      filter: ['all', ['==', '$type', 'Point'], ['==', 'class', 'country']],
      layout: {
        'text-field': '{name:latin}\n{name:nonlatin}',
        'text-font': ['IBMPlexSerif-Regular'],
        'text-max-width': 10,
        'text-size': ['interpolate', ['exponential', 1], ['zoom'], 3, 12, 8, 22],
        'text-allow-overlap': false,
      },
      paint: {
        'text-color': 'hsl(206, 18%, 49%)',
      },
    },
    {
      id: 'railway city',
      type: 'line',
      source: 'openmaptiles',
      'source-layer': 'transportation',
      filter: ['all', ['==', 'class', 'transit']],
      layout: {
        visibility: 'visible',
      },
      paint: {
        'line-color': 'rgba(192, 108, 4, 1)',
        'line-opacity': 1,
      },
    },
    {
      id: 'place_label_city',
      type: 'symbol',
      source: 'openmaptiles',
      'source-layer': 'place',
      minzoom: 5,
      maxzoom: 16,
      filter: ['all', ['==', '$type', 'Point'], ['==', 'class', 'city']],
      layout: {
        'text-field': '{name:latin}\n{name:nonlatin}',
        'text-font': ['IBMPlexSerif-Bold'],
        'text-max-width': 10,
        'text-size': ['interpolate', ['exponential', 1], ['zoom'], 3, 12, 8, 16],
        'text-transform': 'uppercase',
        'text-ignore-placement': false,
        'text-allow-overlap': false,
      },
      paint: {
        'text-color': '#e0e1dd',
        'text-opacity': 0.8,
      },
    },
  ];
}
