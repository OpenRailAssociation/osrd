...
class RawTableSNCF extends Component {
      ...
      <>
          ...
          <td key={head.key}>
            {head.onClick && (
              ...
              <div
                ...
                data-target={
                  head.key === 'supervisor_id' || head.key === 'owner_id' || head.key === 'groups'
                    ? `#${modalId}`
                    : ''
                }
                data-toggle={
            ...
            {head.onClick === undefined && (
              <div className="cell-inner text-monospace">{this.getData(head, item)}</div>
            )}
          </td>
        ))}
        ...
        {actions && (
          <td className="cell-fixed">
            {actions(item.id, item.status, item.layer_name, item.action_type)}
          </td>
        )}
      </>
    );
  ...
  render() {
    const { heads, content, t, actions } = this.props;
          ...
          <div className="table-scroller last-cell-fixed dragscroll">
            <table className="table table-hover">
              <thead className="thead thead-light">
                <tr>
                  {heads.map((head) => (
                    <th scope="col" key={head.key}>
                      <div className="cell-inner">{t(head.label)}</div>
                    </th>
                  ))}
                  {/* <td className="cell-placeholder" aria-hidden="true"><span /></td> */}
                  {actions && (
                    <th className="cell-fixed">
              ...
              <tbody>
                {content.map((item) => (
                  <tr
                    key={item.id}
                    id={item.id}
                    className={item.has_been_restored ? 'table-success' : ''}
                  >
