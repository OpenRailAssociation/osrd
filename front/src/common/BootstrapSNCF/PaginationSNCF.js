...
class PaginationSNCF extends Component {
  ...
  getPages = (pages) => {
    const { activePage, firstPage, getPageContent, paginationRange } = this.props;
    ...
    for (let index = firstPage + 1; index <= limitIndex + firstPage; index += 1) {
      pagination.push(
        <li className={index === activePage ? 'page-item active' : 'page-item'} key={index}>
          <a className="page-link" href={`#${index}`} onClick={(e) => getPageContent(e, pages)}>
            {index}
          </a>
        </li>
      );
  ...
  render() {
    const { t, pages, activePage, getPageContent, centered, paginationRange } = this.props;
    ...
    return (
      <nav role="navigation" className={paginationPositionClasses}>
        <ul className="pagination">
          <li
            className={
              pages === 1 || activePage === 1
                ? 'page-item page-skip disabled'
                : 'page-item page-skip'
            }
          >
            <a
              className="page-link"
              href="#first"
              onClick={(e) => getPageContent(e, pages, paginationRange)}
            >
          ...
          <li
            className={
              activePage === pages ? 'page-item page-skip disabled' : 'page-item page-skip'
            }
          >
            <a
              className="page-link"
