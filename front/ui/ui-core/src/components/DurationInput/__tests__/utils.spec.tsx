...
import {
  ...
  DEFAULT_PADCHAR,
  DEFAULT_MAX,
  DEFAULT_UNIT_KEYS_COMBINATIONS,
  DEFAULT_UNITS,
} from './consts';

describe('utils for useDurationInput', () => {
  describe('normalizeUnits', () => {
    it.each(DEFAULT_UNIT_KEYS)(
      'should carry preset label and msPerUnit onto the output unit',
      (key) => {
        const result = normalizeUnits([key], DEFAULT_PADCHAR, DEFAULT_MAX);
    ...
    it.each(DEFAULT_UNIT_KEYS_COMBINATIONS)(
      ...
      (...units) => {
        const result = normalizeUnits(units, DEFAULT_PADCHAR, DEFAULT_MAX);
  ...
  describe('toUnitField', () => {
    it('for any number, should return the corresponding unit field', () => {
  ...
  describe('toFormattedValues', () => {
    ...
    it('should spread milliseconds into whole units with correct format', () => {
      const result = toFormattedValues(DEFAULT_UNITS, 3_600_000 + 60_000 + 1_000 + 1);
      for (const unit of DEFAULT_UNITS) {
        expect(result[unit.key]).toBe(String('1').padStart(unit.digits ?? 0, unit.padChar));
      }
    });
  });
