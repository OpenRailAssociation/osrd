...
export default function svgToReact(
  ...
  const validateSvgAndExtractPath = (svgFilePath: string, height: string) => {
    ...
    const svgWidth = parseInt(svgElement.attr('width') ?? '0', 10);
    const svgHeight = parseInt(svgElement.attr('height') ?? '0', 10);
    ...
    const viewBox = svgViewBox.match(viewBoxPattern);
  ...
  const representation = svgFiles
    ...
    .reduce<Record<string, Record<string, Record<string, string | undefined>>>>(
  ...
  const sizes = Object.entries(reversed).reduce<Record<string, number>>((acc, [key, value]) => {
