...
describe('useMouseTracking', () => {
  ...
  it.each([
    ...
    { name: 'on the top-left edges', x: 0, y: 0, isHover: true },
    { name: 'on the bottom-right edges', x: WIDTH, y: HEIGHT, isHover: true },
  ...
  ])('should track the mouse when it is $name', ({ x, y, isHover }) => {
    const { result } = renderHook(() => useMouseTracking(createDom()));

    moveMouseTo(x, y);

    expect(result.current).toEqual({ isHover, position: { x, y } });
  });
