...
def _load_infra(editoast_url: str, infra_id: int, session: Session):
    """
    Send a request to preload the infra, raise a RuntimeError on failure
    """
    r = session.post(editoast_url + f"infra/{infra_id}/load")
...
def _reproduce_test(
    ...
    _load_infra(EDITOAST_URL, scenario.infra, session)
