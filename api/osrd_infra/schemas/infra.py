...
class ApplicableDirectionsTrackRange(BaseModel):
    """
    An applicable directions track range is a track range with an associated direction.
    """
...
class SwitchType(BaseObjectTrait):
    ...
    A switch type is define by a list of ports and groups which are the positions that can take the switch.
    """
...
class Catenary(BaseObjectTrait):
    """
    A catenary correspond at a set of cables designed to power electric trains by capturing the current through the use
