...
pub fn parse_osm(osm_pbf_in: PathBuf) -> Result<RailJson, Box<dyn Error + Send + Sync>> {
    ...
    for (node, mut adj) in adjacencies {
        ...
        match (edges_count, branches_count) {
            ...
            (2, 1) => railjson.track_nodes.push(link(node, &adj.branches)),
            (3, 2) => railjson.track_nodes.push(point_switch(node, &adj.branches)),
            (4, 2) => railjson.track_nodes.push(crossing(node, &adj.branches)),
            (4, 4) => railjson
                .track_nodes
...
mod tests {
    ...
    fn parse_track_nodes() {
        ...
        // Track Nodes can be in a random order, we sort them to be sure to extract the expected ones
        railjson
            .track_nodes
