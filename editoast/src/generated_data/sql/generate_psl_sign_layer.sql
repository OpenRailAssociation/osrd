WITH signs AS (
    ...
    WHERE infra_id = $1
        AND osrd_infra_speedsectionmodel.data @? '$.extensions.psl_sncf.z'
    UNION
    SELECT obj_id AS sc_id,
        (
...
collect AS (
    ...
    FROM signs
        INNER JOIN osrd_infra_tracksectionmodel AS tracks ON tracks.obj_id = signs.track_id
...
INSERT INTO osrd_infra_pslsignlayer (obj_id, infra_id, geographic, schematic, data)
SELECT sc_id,
    $1,
    geo,
    sch,
