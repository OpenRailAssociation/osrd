...
/// Context for the track node error generators
#[derive(Debug, Default)]
pub struct Context {
    /// Track endpoint to their track node
...
/// Retrieve invalid track node type refs for track nodes
...
/// Check if track node ports match track node type ports
...
/// Check if the track node ports are not already used by another track node
...
#[cfg(test)]
mod tests {
    use crate::generated_data::error::track_nodes::{check_endpoints_unicity, Context};
    use crate::infra_cache::tests::{
        create_small_infra_cache, create_point_switch_cache, create_track_endpoint,
    };
