...
impl ChannelWorker {
    ...
    pub async fn remove_tracker(&self, correlation_id: &str) {
        let mut response_tracker = self.response_tracker.write().await;
        response_tracker.remove(correlation_id);
    }
...
impl RabbitMQClient {
        ...
        T: Serialize,
    {
        ...
        .await
        {
            ...
            Ok(Err(_)) | Err(_) => {
                channel_worker.remove_tracker(&correlation_id).await;
                Err(MqClientError::ResponseTimeout)
            }
        }
    }
}
