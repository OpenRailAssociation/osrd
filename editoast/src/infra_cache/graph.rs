...
use crate::schema::{TrackNodeCache, TrackEndpoint};
...
pub struct Graph<'a> {
    ...
    /// The group is the group of the track node.
...
impl<'a> Graph<'a> {
    ...
    /// Return the track node linked to the given endpoint.
    ...
    /// Otherwise returns a `Vec` with all the track node groups.
...
mod tests {
    ...
    fn get_track_node() {
        ...
        assert_eq!(track_node.obj_id, "point_switch".to_string());
