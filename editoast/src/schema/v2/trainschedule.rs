...
impl<'de> Deserialize<'de> for MarginValue {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if value.ends_with("min/km") {
            let float_value: f64 = f64::from_str(&value[0..value.len() - 6]).map_err(|_| {
                serde::de::Error::invalid_value(
                    serde::de::Unexpected::Str(&value),
                    &"a valid float",
                )
...
mod tests {
    ...
    fn deserialize_margins() {
        let valid_margins = r#"{"boundaries":["a", "b"],"values":["0","10%","20min/km"]}"#;
        assert!(from_str::<Margins>(valid_margins).is_ok());
        let invalid_margins = r#"{"boundaries":["a"],"values":["0","10%","20min/km"]}"#;
