...
struct RoutesFromNodesPositions {
    routes: Vec<String>,
    available_node_positions: HashMap<String, Vec<String>>,
}
...
async fn get_routes_nodes(
    ...
    if node_states.is_empty() {
        return Ok(Json(RoutesFromNodesPositions {
            routes: vec![],
            available_node_positions: HashMap::new(),
        }));
    }
...
mod tests {
    ...
    async fn get_routes_nodes() {
        let tests = vec![
            ...
            (
                ...
                (
                    ...
                    vec![("PA2".to_string(), vec!["A_B2".to_string()])],
                ),
            ),
            (
                ...
                (
                    ...
                    vec![("PD0".to_string(), vec!["STATIC".to_string()])],
                ),
            ),
            (
                ...
                (
                    ...
                    vec![("PH0".to_string(), vec!["A1_B1".to_string()])],
                ),
            ),
            (
                ...
                (
                    ...
                    vec![(
                        "PH1".to_string(),
                        vec!["A_B1".to_string(), "A_B2".to_string()],
                    )],
                ),
            ),
            (
                ...
                (
                    ...
                    vec![
                        ("PA0".to_string(), vec!["A_B1".to_string()]),
                        ("PA2".to_string(), vec!["A_B1".to_string()]),
                    ],
                ),
            ),
            (
        ...
        fn compare_result(got: RoutesFromNodesPositions, expected: RoutesFromNodesPositions) {
            ...
            let process_available_positions = |positions: &HashMap<String, Vec<String>>| {
                positions
                    .iter()
                    .map(|(key, value)| {
                        let mut sorted_node_positions = value.clone();
            ...
            let mut got_available_positions: Vec<_> =
                process_available_positions(&got.available_node_positions);
            let mut expected_available_positions: Vec<_> =
                process_available_positions(&expected.available_node_positions);

            got_available_positions.sort();
            expected_available_positions.sort();
