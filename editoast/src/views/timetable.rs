...
pub(in crate::views) async fn conflicts(
    ...
    let train_ids: Vec<_> = paced_train_ids
        .into_iter()
        .chain(trains.iter().map(|ts| TrainId::new_single(ts.id)))
        .collect();
...
mod tests {
    ...
    fn build_coherent_conflict_core_request() {
        ...
        // Train IDs
        let train_ids = vec![
            TrainId::new_occurrence(paced_train_id, 0),
            TrainId::new_occurrence(paced_train_id, 1),
            TrainId::new_single(ts_id),
        ];
        ...
        let simple_ts_train_core_id = trains_ids_map
            ...
            .find_map(|(core_id, train_id)| match train_id {
                TrainId::Single { id } if *id == ts_id => Some(core_id),
                _ => None,
            })
            .unwrap();
        ...
        let paced_0_train_core_id = trains_ids_map
            .iter()
            .find_map(|(core_id, train_id)| match train_id {
                TrainId::Occurrence { id, index, .. } if *id == paced_train_id && *index == 0 => {
                    Some(core_id)
                }
                _ => None,
            })
        ...
        let paced_1_train_core_id = trains_ids_map
            .iter()
            .find_map(|(core_id, train_id)| match train_id {
                TrainId::Occurrence { id, index, .. } if *id == paced_train_id && *index == 1 => {
                    Some(core_id)
                }
                _ => None,
            })
