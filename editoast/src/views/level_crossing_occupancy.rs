...
pub(in crate::views) async fn occupancy(
    ...
    let train_occurrences = trains
        ...
        .flat_map(|train| {
            ...
            train.iter_occurrences(&train_exceptions).collect_vec()
        })
        .collect_vec();

    let rolling_stock_names: HashSet<_> = trains
        .iter()
        .map(|train| train.rolling_stock_name.clone())
        .collect();
    ...
    let train_occurrences = filter_readable_occurrences(
        train_occurrences.into_iter().collect(),
        &rolling_stocks,
        &authn_state,
        regulator.openfga(),
    )
