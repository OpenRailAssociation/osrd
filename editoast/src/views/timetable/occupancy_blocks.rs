...
impl Cachable for TrainBlockOccupancyDetails {
    ...
    fn key(&self, app_version: &str) -> String {
        ...
        let infra_id = self.infra_id;
        let infra_version = self.infra_version;
        format!("occupancy_block_{app_version}.{infra_id}.{infra_version}.{hash_simulation_input}")
    }
}
...
pub(super) async fn compute_occupancy_blocks<T: TrainScheduleLike>(
    ...
    let filtered = extract_block_occupancy_details(
        simulations,
        trains_schedules,
        Arc::new(path.clone()),
        infra,
    ...
    let cache = core_task::BufCache::new(valkey_client.clone(), filtered.len());
    ...
    let (cache_keys, cached_blocks) = cache.fetch_by_inputs::<_, OccupancyBlocks>(&filtered).await;
    ...
    for ((key, _), signal_updates) in itertools::izip!(occupancy_block_requests, signal_updates) {
        cache.batched_write(key.to_owned(), serde_json::to_value(&signal_updates)?);
        let indexes = &train_hashes_to_idx[key];
