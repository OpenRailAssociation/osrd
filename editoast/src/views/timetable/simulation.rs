...
pub fn build_simulation_train(
    ...
    for (track_offsets, PathItem { key, .. }) in track_offsets.iter().zip(path) {
        let (at, simulation_schedule_item, can_backtrack) = match schedule_map.get(key) {
            None => (key, core_task::ScheduleItem::pass_by(), false),
            Some(ScheduleItem {
                at,
                arrival,
                stop_for,
...
pub fn build_sim_schedule_items(
    ...
    let path_item_ids: HashSet<_> = path_items.iter().map(|item| &item.key).collect();
    assert!(
        schedule_items
            .iter()
        ...
        assert!(
            backtrack_path_item_indexes
                ...
                .all(|&index| index < path_items.len()
                    && schedule_items_ids.contains(&path_items[index].key)),
        )
    }
