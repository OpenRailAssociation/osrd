...
fn render_success(
    ...
    let xml = to_string(&msg).map_err(|e| TsiError::XmlSerialize(e.to_string()))?;
...
fn build_identifiers(prm: &PathRequestMessage) -> Result<PdmIdentifiers, TsiError> {
    ...
    // TODO: Should we generate it ?
    let id_core = Uuid::new_v4().to_string().replace('-', "").to_uppercase()[..12].to_string();
...
fn build_pdm_locations(
    ...
    let arrival_time = departure_time
        + Duration::milliseconds(path_item_times.get(index).copied().unwrap_or(0) as i64);
    ...
    let planned_train_data = prm_loc
        .planned_train_data
        .as_ref()
        .map(|d| PdmPlannedTrainData {
