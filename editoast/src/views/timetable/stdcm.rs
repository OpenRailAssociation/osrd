...
mod tests {
    use editoast_schemas::rolling_stock::{
        Gamma, RollingResistance, RollingStock, TowedRollingStock,
    };
    ...
    fn simulation_with_towed_rolling_stock_parameters() {
        ...
        rolling_stock.name = "BB 22200 22200G-911".to_string();
        rolling_stock.mass = 89000.0;
        rolling_stock.inertia_coefficient = 1.16;
        rolling_stock.comfort_acceleration = 0.08;
        rolling_stock.startup_acceleration = 0.03;
        rolling_stock.rolling_resistance =
            RollingResistance::new("davis".to_string(), 1.1, 0.0089, 0.0003);

        let towed_rolling_stock = create_towed_rolling_stock();

        // Traction engine mass + Towed Mass (empty) + Masse of the train payload
        let total_mass = 89000.0 + 46000.0 + 30000.0;
        ...
        // RAV X(A, B, C) : X_engin_traction * masse_engin_traction + X_mat_remorqué * (masse_convoi - masse_engin_traction)
        assert_eq!(
            physics_rolling_stock.rolling_resistance,
            RollingResistance::new("davis".to_string(), 163229.60000000003, 1370.46, 39.924)
        );

        // Inertie : Inertie de l'engin de traction (masse_engin_traction * coeff_inertie_engin_traction) + inertie du matériel remorqué (total_masse_convoi - masse_engin_traction) * coeff_inertie_mat_remorqué)
        assert_eq!(
            physics_rolling_stock.inertia_coefficient,
            ((rolling_stock.mass * rolling_stock.inertia_coefficient)
                + ((total_mass - rolling_stock.mass) * towed_rolling_stock.inertia_coefficient))
                as f64
        );
    }
    ...
    fn simulation_with_parameters() {
        ...
        let physics_rolling_stock: PhysicsRollingStock =
            PhysicsRollingStock::new(rolling_stock.into(), None, simulation_parameters);
