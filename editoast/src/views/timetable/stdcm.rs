...
async fn stdcm(
    ...
    let (
        simulation_run_time,
        fake_train_schedule,
        fake_train_sim_result,
        fake_train_pathfinding_result,
    ) = simulate_train_run(
        db_pool.clone(),
        valkey_client.clone(),
        core_client.clone(),
...
async fn handle_path_not_found(
    fake_train_schedule: TrainSchedule,
    train_schedules: Vec<TrainSchedule>,
    simulations: Vec<(SimulationResponse, PathfindingResult)>,
    fake_train_sim_result: SimulationResponse,
    fake_train_pathfinding_result: PathfindingResult,
    ...
    let fake_train_id = fake_train_schedule.id;

    // Combine the original train schedules with the fake train schedule.
    let train_schedules = [train_schedules, vec![fake_train_schedule]].concat();

    // Combine the original simulations with the fake train's simulation results.
    let simulations = [
        simulations,
        vec![(fake_train_sim_result, fake_train_pathfinding_result.clone())],
    ]
    .concat();
    ...
    // Filter the conflicts to find those specifically related to the fake train.
    let conflicts: Vec<_> = conflict_detection_response
        .conflicts
        .into_iter()
        .filter(|conflict| conflict.train_ids.contains(&fake_train_id))
        .map(|mut conflict| {
            conflict.train_ids.retain(|id| id != &fake_train_id);
            conflict
        })
        .collect();

    // Return the conflicts found along with the pathfinding result for the fake train.
    Ok(STDCMResponse::Conflicts {
        pathfinding_result: fake_train_pathfinding_result,
        conflicts,
    })
}
