...
pub(in crate::views) async fn conflicts(
    ...
    let train_schedules_with_exceptions = match &authn_state {
        authentication::State::Skip => train_schedules_with_exceptions,
        authentication::State::Authenticated { .. } => {
            ...
            // Authorize each rolling stock separately
            let authorizer = authn_state.authorizer(&openfga);
            let accesses = authorizer
                .authorize_all(rolling_stocks.into_iter().map(|rolling_stock| {
                    v2::rolling_stock_privilege_check(
                        authz::RollingStock(rolling_stock.id),
                        RollingStockPrivilege::CanRead,
                    )
                    .map(async move |()| rolling_stock.name)
                }))
                .await?;
            let authorized_rolling_stock_names: HashSet<String> = v2::Access::access_all(accesses)
                .await?
                .into_iter()
