...
use crate::schema::osrd_infra_operationalpointlayer;
use crate::schema::osrd_infra_operationalpointlayer::dsl::*;
...
impl OperationalPointLayer {
    /// Clear and regenerate fully the operational point layer of a given infra id
    pub fn refresh(
        conn: &PgConnection,
        infra: i32,
        chartos_config: &ChartosConfig,
    ) -> Result<(), Error> {
        delete(osrd_infra_operationalpointlayer.filter(infra_id.eq(infra))).execute(conn)?;
        sql_query(include_str!("sql/generate_operational_point_layer.sql"))
            .bind::<Integer, _>(infra)
            .execute(conn)?;
        invalidate_chartos_layer(infra, "operational_points", chartos_config);
        Ok(())
    }
    ...
    pub fn delete_list(
        ...
        sql_query(
            "DELETE FROM osrd_infra_operationalpointlayer WHERE infra_id = $1 AND obj_id = ANY($2)",
        )
        .bind::<Integer, _>(infra)
        .bind::<Array<Text>, _>(&obj_ids)
        .execute(conn)?;
    ...
    fn fill_operational_point_track_refs(
        infra_cache: &InfraCache,
        track_id: &String,
        results: &mut HashSet<String>,
    ) {
        infra_cache
            .get_track_refs_type(track_id, ObjectType::OperationalPoint)
            .iter()
            .for_each(|obj_ref| {
    ...
    /// Search and update all operational points that needs to be refreshed given a list of operation.
    pub fn update(
        conn: &PgConnection,
        infra: i32,
        ...
        // For this layer we can't avoid updating it if a track reference (in parts) is invalid.
        ...
        for op in operations {
            match op {
                ...
                | OperationResult::Update(RailjsonObject::TrackSection { railjson }) => {
                    Self::fill_operational_point_track_refs(
                        infra_cache,
                        &railjson.id,
                        &mut obj_ids,
                    );
                ...
                OperationResult::Delete(ObjectRef {
                    ...
                    obj_id: operational_point_id,
                }) => {
                    obj_ids.insert(operational_point_id.clone());
                }
                OperationResult::Delete(ObjectRef {
                    obj_type: ObjectType::TrackSection,
                    obj_id: track_id,
                }) => {
                    Self::fill_operational_point_track_refs(infra_cache, track_id, &mut obj_ids);
                }
                _ => (),
            }
        }
        ...
        invalidate_bbox_chartos_layer(infra, "operational_points", invalid_zone, chartos_config);
        Ok(())
    }
}
