...
use crate::schema::osrd_infra_routelayer;
use crate::schema::osrd_infra_routelayer::dsl::*;
...
impl RouteLayer {
    /// Clear and regenerate fully the route layer of a given infra id
    pub fn refresh(
        conn: &PgConnection,
        infra: i32,
        chartos_config: &ChartosConfig,
    ) -> Result<(), Error> {
        delete(osrd_infra_routelayer.filter(infra_id.eq(infra))).execute(conn)?;
        sql_query(include_str!("sql/generate_route_layer.sql"))
            .bind::<Integer, _>(infra)
            .execute(conn)?;
        invalidate_chartos_layer(infra, "routes", chartos_config);
        Ok(())
    }
    ...
    pub fn insert_list(
        ...
        sql_query(include_str!("sql/insert_route_layer.sql"))
            .bind::<Integer, _>(infra)
    ...
    pub fn delete_list(
        ...
        sql_query("DELETE FROM osrd_infra_routelayer WHERE infra_id = $1 AND obj_id = ANY($2)")
            .bind::<Integer, _>(infra)
            .bind::<Array<Text>, _>(&obj_ids)
            .execute(conn)?;
    ...
    fn fill_route_track_refs(
        infra_cache: &InfraCache,
        track_id: &String,
        results: &mut HashSet<String>,
    ) {
        infra_cache
            .get_track_refs_type(track_id, ObjectType::Route)
            .iter()
            .for_each(|obj_ref| {
    ...
    /// Search and update all routes that needs to be refreshed given a list of operation.
    pub fn update(
        conn: &PgConnection,
        infra: i32,
        ...
        for op in operations {
            match op {
                ...
                | OperationResult::Update(RailjsonObject::TrackSection { railjson }) => {
                    Self::fill_route_track_refs(infra_cache, &railjson.id, &mut obj_ids);
                }
                ...
                OperationResult::Delete(ObjectRef {
                    ...
                    obj_ids.insert(route_id.clone());
                }
                OperationResult::Delete(ObjectRef {
                    obj_type: ObjectType::TrackSection,
                    obj_id: track_id,
                }) => {
                    Self::fill_route_track_refs(infra_cache, track_id, &mut obj_ids);
                }
                _ => (),
            }
        }
        ...
        invalidate_bbox_chartos_layer(infra, "routes", invalid_zone, chartos_config);
        Ok(())
    }
}
