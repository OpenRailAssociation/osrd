...
use super::{QueueStateMap, TargetTracker, GenerationId};
...
async fn next_event(state: &TargetTracker, inbox: &mut ActorInbox) -> LoopEvent {
...
pub async fn target_tracker_actor(
    ...
    let mut state = TargetTracker::new(Instant::now(), initial_state, config.unbind_delay, config.delete_delay);
    ...
    while running {
        ...
        if let Some(msg) = msg {
            match msg {
                ...
                ActorMessage::RequireQueue { key, extra_lifetime, respond_to } => {
