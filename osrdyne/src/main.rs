...
fn request_queues_policy(config: &OsrdyneConfig) -> BTreeMap<String, ArgumentValue> {
    ...
    if let Some(default_message_ttl) = config.default_message_ttl {
        request_queues_policy.insert("message-ttl".into(), ArgumentValue::Int(default_message_ttl));
    }
    ...
    if let Some(max_length_bytes) = config.max_length_bytes {
        request_queues_policy.insert("max-length-bytes".into(), ArgumentValue::Int(max_length_bytes));
    }
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let pool = Arc::new(Pool::new(config.pool_id.clone(), request_queues_policy(&config)));
    ...
    let init_keys: Vec<Key> = init_queues.into_iter().filter_map(|q| pool.parse_key(&q.name)).collect();
    ...
    let mut target_tracker = spawn(target_tracker_actor(tracker_inbox, init_keys, target_tracker_config));

    target_tracker_client.require_worker_group(Key::new("test"), Duration::ZERO).await?;
    ...
    'reconnect_loop: loop {
        ...
        let mut tasks = pool.start(&conn, &management_client, target_tracker_client.clone()).await?;
