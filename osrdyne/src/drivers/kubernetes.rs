    ...
    fn get_or_create_worker_group(
        ...
        Box::pin(async move {
            ...
            for worker in current_workers {
                ...
                if worker.worker_key == worker_key && forced_id.is_none() {
                    // It's not an update, and we found a worker with the same key
                    return Ok(worker.worker_id);
                } else if worker.worker_key == worker_key
                    && worker_version == self.version_identifier
                {
    ...
    fn refresh_workers(
        ...
        Box::pin(async move {
            ...
            for worker in current_workers.clone() {
                let worker_version_identifier = worker
                    .metadata
                    .get(LABEL_VERSION_IDENTIFIER)
                    .expect("version_identifier not found")
                    .to_owned();

                if worker_version_identifier != self.version_identifier {
                    let worker_id = worker.worker_id;
                    let worker_key = worker.worker_key.clone();
                    ...
                    info!(
                        "Refreshing deployment {} due to version mismatch",
                        worker.worker_key.to_string()
                    );

                    self.get_or_create_worker_group(queue_name, worker_key, Some(worker_id))
                        .await?;
                }
            }
