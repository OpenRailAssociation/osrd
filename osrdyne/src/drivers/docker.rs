...
impl WorkerDriver for DockerDriver {
    ...
    fn refresh_workers(
        ...
        Box::pin(async move {
            ...
            for worker in current_workers {
                let current_version = worker
                    .metadata
                    .get(LABEL_VERSION_IDENTIFIER)
                    .expect("version_identifier metadata missing")
                    .to_owned();

                if current_version != self.version_identifier {
                    log::info!(
                        "Removing container {} due to version mismatch",
                        worker.external_id
                    );
                    ...
                    self.get_or_create_worker_group(
